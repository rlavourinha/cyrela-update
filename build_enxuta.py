...
body += _obox36(f'Lançar não é faturar, mas fatura mais cedo: projetos novos são {fmt(100 * nov_ltm / REC_LTM, 0)}% da receita e entram com PoC de ~{fmt(poc_n4, 0)}% (~{fmt(poc_4t, 0)}% no 4T).',
                f'Desde o 4T25 a renúncia ao direito de desistir sai a cada fechamento: o projeto entra antes, com menos obra e menos vendido; a usinagem sobre os antigos é o piso de {fmt(100 * usin_ant / REC_LTM, 0)}%, não os {fmt(100 * eU / REC_LTM, 0)}% do modelo antigo.')
...
_s48 = sl("parte 4 · onde estamos no ciclo", f"Cury: {fmt(_n2['pb_cury'], 1)}x book com ROE de {fmt(_n2['roe_cury'], 0)}%; a Vivaz valeria {fmt(_n2['pb_viv'], 1)}x.",
    ...
    + _obox(f'Alavancagem da Cury é operacional: caixa líquido, terreno a prazo em {fmt(_n2["cred_pl"], 0)}% do PL, ativo {fmt(_n2["at_pl"], 1)}x o PL; ROE {fmt(_n2["roe_cury"], 0)}% vale {fmt(_n2["pb_cury"], 1)}x book (Gordon {fmt(_n2["pb_cury_g"], 1)}x).',
            f'Vivaz: retorno operacional {fmt(_n2["roe_viv"] / _n2["fator_ll"], 0)}%, ROE ~{fmt(_n2["roe_viv"], 0)}%, valeria {fmt(_n2["pb_viv"], 1)}x book, R$ {fmt(_n2["val_viv"], 1)} bi, {fmt(100 * _n2["val_viv"] / _n2["mc_cyre"], 0)}% do valor da Cyrela com 8% do PL; a fatia na Cury vale R$ {fmt(_n2["stake"], 1)} bi.'),
    nota=f"Fontes: Cury, planilha Fundamentos do RI (DRE e balanço trimestrais, consolidado): retorno operacional = lucro antes do resultado financeiro LTM ÷ PL total médio de 5 pontas; ROE = lucro atribuído aos controladores LTM ÷ PL da controladora médio; dívida líquida = empréstimos e financiamentos − caixa − títulos; terreno a prazo = credores por imóveis compromissados. B3 ({_n2['mkt']['data']}): CURY3 R$ {fmt(_n2['mkt']['CURY3'], 2)}, {fmt(_n2['mkt']['acoes_cury'] / 1e6, 1)} mi ações; CYRE3 R$ {fmt(_n2['mkt']['CYRE3'], 2)}, {fmt(_n2['mkt']['acoes_cyre'] / 1e6, 1)} mi ações. Vivaz: nota de segmentos do ITR (2T26), sem juros, IR e minoritários alocados; ROE estimado = retorno operacional × {fmt(_n2['fator_ll'], 2)} (razão ROE ÷ retorno operacional da Cury). P/B implícito = (ROE − g) ÷ (Ke − g), Ke 17% e g 4%, premissas do slide do P/B; a Cury a mercado ({fmt(_n2['pb_cury'], 1)}x) confirma a régua. Não é preço-alvo: é o que o mercado pagaria pela Vivaz se ela fosse listada com o retorno de hoje.").replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
...
_NOVOS = ("Lucro, caixa, dívida e payout", "Tecnisa: R$ 95 mi de equity", "Médio e alto padrão: o mercado desacelera", "MCMV: o mercado segue no recorde", "Banco a banco: a Caixa carrega", "Share no crédito habitacional sem FGTS", "Estouro no MAP", "Estouro no MCMV", "Cury × Vivaz", "Receita por trimestre", "SP no mapa", "Retorno por vertical", "x book com ROE")
for _k, _s in enumerate(final):
