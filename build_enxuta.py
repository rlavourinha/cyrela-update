...
body = svg(980, 212, c.flush() + '<text x="60" y="18" class="gtit">Lançamentos por região (VGV 100%, R$ bi por ano)</text><text x="60" y="34" class="gsub">planilha operacional do RI; "outras" = MG, ES, Norte, Nordeste e Centro-Oeste · LTM = 3T25-2T26, em tom claro</text>')
...
slides.append(take(40, P1, title="Velocidade de venda contra o mercado (Geoimóvel, SP).",   # 23/09/26: numa linha a 38px (919px); "— base Geoimóvel, SP capital" media 1136px; a nota diz base e data
  callout='',
  # 23/09/26 (agente de formatação): o slide media 880px. Os três painéis de "vendido por idade" tinham 39 unidades vazias entre a linha de 40% (y=175+22) e a
  # linha-base (y=214+22): linha-base sobe para 190, rótulos do eixo para 206 e legendas "n=" para 228; viewBox de 292 para 266. Os dois svgs ficam a 900px
  # (escala 1:1 em vez de 1,13) e a caixa verde vai ao padrão (mensagem numa linha, apoio em largura inteira; antes o apoio abria 4 linhas numa coluna estreita).
  repl=[('<svg viewBox="0 0 900 292">', '<svg viewBox="0 0 900 266">'), ('y1="214.0"', 'y1="190.0"'), ('y2="214.0"', 'y2="190.0"'), ('y="230.0" class="axq"', 'y="206.0" class="axq"'), ('y="252" class="fw-s2"', 'y="228" class="fw-s2"'),
        ('<div class="viz"><svg viewBox="0 0 900 266">', '<div class="viz" style="max-width:900px;margin:0 auto"><svg viewBox="0 0 900 266">'), ('<div class="viz"><svg viewBox="0 0 900 212">', '<div class="viz" style="max-width:900px;margin:2px auto 0"><svg viewBox="0 0 900 212">')],
  append=_obox18('A Cyrela não opera o ciclo; opera o presente.', 'Não há um fair share perseguido: a companhia vai sentindo o mercado e, conforme vende bem, aumenta o pipeline de projetos. É bom no sentido de não congelar na loucura que é o Brasil — e é o que dá mais problema quando o ciclo vira, porque o pipeline montado no topo chega à obra e à entrega na descida.', px=19)
  + '<p class="sl-nota">Base: Geoimóvel, SP capital, foto de mai/26, comparação na mesma idade (% vendido por faixa de meses desde o lançamento; mercado ex-Cyrela do mesmo segmento).</p>').replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1))   # 22/09/26: base e data na nota (decisão editorial do autor)
...
body += _obox18('Lucro 12m de R$ ' + fmt(ll12[_i22], 1) + ' bi (2022) para ' + fmt(_llL, 1) + ' bi com PL +' + fmt(_plg, 0) + '% e dívida líquida em ' + fmt(_plL, 0) + '% do PL, sem emissão.', 'O balanço não inchou: o MCMV gira com pouco capital (a CEF financia a obra sobre o vendido) e o terreno entra a prazo e em permuta — a dívida líquida de ' + fmt(_plL, 0) + '% do PL é ex-terrenos a pagar. O dividendo de R$ ' + fmt(_sum4(DIV_Q, qs_f[-1], False) / 1000, 1) + ' bi saiu dessa folga, não do caixa operacional de R$ ' + fmt(_gcL, 1) + ' bi.')   # 22/09/26: mensagem no formato do autor (decisão editorial); caixa a 18px, tag alinhada à 1ª linha; 23/09/26: "(ex-terrenos a pagar)" desce para o apoio (a mensagem media 1011px a 18px e abria 2 linhas; agora 827px)
...
_WLAB = {_W_HOJE: " (hoje, 2T26)", _W_CEN: " (cenário: o triplo)", 0.5: ""}   # 23/09/26: "(hoje, nota de segmentos 2T26)" abria 2 linhas na 1ª coluna da tabela; a nota de segmentos fica na nota da tabela
...
# 23/09/26 (agente de formatação): o slide media 805px; título numa linha a 38px (930px; o original, "passa anos debaixo d'água. O do MCMV quase não afunda", media 1107px)
_s8 = re.sub(r'<h2 class="head-xl" style="font-size:38px">.*?</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">O caixa do MAP afunda por anos; o do MCMV quase não.</h2>', _s8, count=1, flags=re.S)
...
_s8_add = ('<div class="fwgrid" style="grid-template-columns:1.15fr 1fr;margin-top:6px;gap:10px;align-items:start">' + re.sub(r' style="[^"]*"', ' style="margin-top:0"', _viz41, count=1)
    ...
    + '<div class="viz" style="margin-top:6px"><table class="tl" style="width:100%;font-size:10px;line-height:1.1;border:2px solid var(--s2);border-collapse:collapse;text-align:center"><thead>'
       ...
       + f'</tbody></table><p class="sl-nota" style="margin:2px 2px 0;color:var(--s2)">Cenários: retorno operacional (antes de juros e IR); w = parcela do PL atribuído à Vivaz (11% no 2T26, nota de segmentos); MAP = alto padrão nos níveis que o ciclo já mostrou. Com um terço na Vivaz e o MAP no fundo de 2018, o consolidado cai a {fmt(_z18, 1)}% ({fmt(_z18_lo, 1)}% se a Vivaz cair a 30%).</p></div>'   # 23/09/26: nota numa linha (os +3,6/+1,5 p.p. já estão na caixa verde; "o mix segura, não repõe" está no apoio)
    + _obox18(f'Mix ajuda, ciclo manda: Vivaz a 33% do capital vale +{fmt(_mix_lo, 1)} a {fmt(_mix_hi, 1)} p.p.; o MAP no fundo de 2018 leva a ~{fmt(_z18, 0)}%.',   # 23/09/26: 857px a 18px, uma linha ("de 11% para 33% ... derruba o retorno" media 930px e abria 2 linhas); o 11% de hoje está no apoio e na tabela
...
slides.append(sl(P5, "Usinagem: o que a receita deve à obra, e por que se erra.", body, nota="Fontes: DRE (CYREMod, receita líquida trimestral); planilha do RI (vendas de lançamento, de estoque em construção e de estoque pronto, VGV 100%); ITR (nota de estoques: imóveis a comercializar em construção; nota de obras em andamento: receita total de vendas e apropriada); releases (custo orçado a incorrer das unidades em estoque), em _usinagem.json. Estimativa com as premissas do slide anterior, mas com o PoC do estoque de cada trimestre (o slide anterior usa o de jun/26); PoC interpolado em 3T21 e 4T22.").replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1))   # 23/09/26: título numa linha a 38px (a 46px abria duas)
...
slides.append(sl(P5, '<span style="font-size:38px">Por segmento: a Vivaz já é um quinto do lucro bruto.</span>', body, nota="Fontes: CYREMod linhas 35-42 (nota de informações por segmento dos ITR/DFP, 1T20-2T26); pares: releases 1S26 (analise_fr_e_decks.md §8.1)."))
...
# 23/09/26 (agente de formatação): três slides gerados ganharam título mais curto (uma linha a 38px); a seção publicada de mesmo tema continua sendo substituída por eles
_gen["Velocidade de venda contra o mercado — base Geoimóvel, SP capital."] = next(s for s in slides if "Velocidade de venda contra o mercado" in _h2(s))
_gen["Usinagem: o que a receita deve à obra, e por que ela é difícil de acertar."] = next(s for s in slides if "Usinagem: o que a receita deve" in _h2(s))
_gen["O caixa do MAP passa anos debaixo d'água. O do MCMV quase não afunda."] = _s8
_static_titles = set()
final = []
...
_REPL_TXT = [
    ("A renda cresceu — e foi ela, não o crédito, que sustentou a demanda.", "A renda cresceu, mas o 3x veio do topo e do investidor."),   # 22/09/26: título alinhado à caixa verde (revisão contrarian); 23/09/26: encurtado para caber numa linha a 38px (a versão "mas não explica o 3x: a demanda veio do topo e do investidor" media 1293px > 1020)
 ...
 # ---- agente de formatação (23/09/26): títulos numa linha a 38px, caixas verdes no padrão (mensagem numa linha + apoio embaixo), rótulos de svg sem sobreposição
 # slide 5 (dono/bônus): título sem <br> (cabe numa linha a 46px); o svg de 900×120 tinha o título (y=20) em cima de "40", "inclui ações 20" e "29,2" → viewBox com 14 unidades de folga no topo e títulos em y=6; caixa verde com apoio em largura inteira (antes: 7 linhas numa coluna estreita)
 ('<h2 class="head-xl">O dono, o bônus<br>e o espólio.</h2>', '<h2 class="head-xl">O dono, o bônus e o espólio.</h2>'),
 ('<div class="viz"><svg viewBox="0 0 900 120">\n<text x="70" y="20" class="gtit">Diretoria estatutária: total (R$ mi)</text>', '<div class="viz"><svg viewBox="0 -14 900 134">\n<text x="70" y="6" class="gtit">Diretoria estatutária: total (R$ mi)</text>'),
 ('<text x="500" y="20" class="gtit">Por diretor: maior · média · menor (R$ mi)</text>', '<text x="500" y="6" class="gtit">Por diretor: maior · média · menor (R$ mi)</text>'),
 ('<div class="sl-output"><span class="out-tag">o que fica</span><span class="out-msg">O alinhamento é o dono na mesa — e o dono está saindo de cena.</span><span class="out-sub">A diretoria atual ganha dinheiro pela primeira vez (R$ 3-6 mi por ano em 2017-20 e R$ 15-22 mi desde 2021): o pool só secou em 2015-16, os anos de prejuízo, e isso pesa na virada do ciclo. Os filhos: <b>Raphael</b> é o mais próximo do mercado, cabeça de financista, quem advoga pela baixa renda; <b>Efraim</b> é mais afastado, gosta do luxo e das parcerias com grandes marcas.</span></div>',
  _obox36("O alinhamento é o dono na mesa — e o dono está saindo de cena.", "A diretoria ganha dinheiro pela primeira vez (R$ 3-6 mi/ano em 2017-20, R$ 15-22 mi desde 2021); o pool só secou em 2015-16, os anos de prejuízo, e isso pesa na virada do ciclo. <b>Raphael</b> é o mais próximo do mercado, financista, advoga pela baixa renda; <b>Efraim</b>, mais afastado, gosta do luxo e das parcerias com grandes marcas.")),
 # slide 6 (caixas pretas): título numa linha a 38px (995px; a 46px abria duas linhas)
 ('<h2 class="head-xl">Três caixas pretas: Tecnisa, a torre com quatro galpões, e a SKR.</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:6px">Três caixas pretas: Tecnisa, torre com quatro galpões e SKR.</h2>'),
 # slide 9 (recebível performado): 879px → título a 38px numa linha, gráfico a 880px de largura, cartões na variante compacta, sem a frase repetida ("a companhia não abre") e sem "no trimestre" duplicado, cabeçalho do cartão da direita numa linha, nota do cartão mais curta (mesmos números), caixa verde no padrão (mensagem numa linha a 19px, apoio embaixo)
 ('<h2 class="head-xl">Recebível performado: a Cyrela também financia o cliente.</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">Recebível performado: a Cyrela também financia o cliente.</h2>'),
 ('Recebível performado: a Cyrela também financia o cliente.</h2><div class="viz"><svg viewBox="0 0 980 200">', 'Recebível performado: a Cyrela também financia o cliente.</h2><div class="viz" style="max-width:880px;margin:0 auto"><svg viewBox="0 0 980 200">'),
 ('<div class="fwgrid" style="margin-top:10px"><div class="fwcard map"><header>como funciona</header>', '<div class="fwgrid compact" style="margin-top:6px"><div class="fwcard map"><header>como funciona</header>'),
 ('3-8% do lucro bruto. A companhia não abre o valor (pergunta 27). Sensibilidade: <b>1 p.p. de IPCA a mais no trimestre ≈ +30 bps de margem bruta</b> no trimestre (R$ 8-16 mi', '3-8% do lucro bruto. Sensibilidade: <b>1 p.p. de IPCA a mais no trimestre ≈ +30 bps de margem bruta</b> (R$ 8-16 mi'),
 ('<header>retorno sobre o custo · por R$ 100 financiados (premissas)</header>', '<header>retorno por R$ 100 financiados (premissas)</header>'),
 ('<p class="sl-nota" style="margin:4px 0 0">O crédito é "alavancado": o juro corre sobre o preço, mas o capital da Cyrela é o custo. Com alienação fiduciária a inadimplência devolve o imóvel (LGD baixa, como nos CRIs da CashMe); no pró-soluto a inadimplência é perda. Aguardando repasse não rende juro, só INCC. PDD de R$ 96 mi sobre R$ 6,5 bi (líquido; R$ 7,1 bi apropriados antes das provisões: 1,3%); o FR 2026 criou fator de risco para a inadimplência pós-conclusão.</p>',
  '<p class="sl-nota" style="margin:4px 0 0">Crédito "alavancado": o juro corre sobre o preço, o capital da Cyrela é o custo. Na alienação fiduciária a inadimplência devolve o imóvel (LGD baixa, como nos CRIs da CashMe); no pró-soluto é perda; aguardando repasse rende só INCC. PDD de R$ 96 mi sobre R$ 6,5 bi (líquido; 1,3% dos R$ 7,1 bi apropriados antes das provisões); o FR 2026 criou fator de risco para a inadimplência pós-conclusão.</p>'),
 ('<div class="sl-output"><span class="out-tag">o que fica</span><span class="out-msg">Risco no balanço, retorno perto do custo de capital — mas é a engrenagem que garante a venda.</span><span class="out-sub">R$ 1,6 bi a 12% mais inflação: remunera em linha ou pouco acima do custo de capital e traz risco de crédito para dentro da incorporadora; existe porque segura a venda quando o banco não repassa.</span></div>',
  _obox36("Risco no balanço, retorno perto do custo de capital — mas é a engrenagem que garante a venda.", "R$ 1,6 bi a 12% mais inflação: remunera perto do custo de capital e traz risco de crédito para a incorporadora; existe porque segura a venda quando o banco não repassa.")),
 # slide 13 (parcerias): título a 38px numa linha; "SPEs em equivalência" invadia o "1.124" na tabelinha do svg; tabela com larguras fixas e 11px para nenhuma célula abrir 3 linhas (a fonte do mútuo já está na nota de rodapé: FR 16.2)
 ('<h2 class="head-xl">Parcerias: três viraram listadas, uma perde dinheiro.</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">Parcerias: três viraram listadas, uma perde dinheiro.</h2>'),
 ('<text x="0" y="83" class="fw-t2">SPEs em equivalência</text>', '<text x="0" y="83" class="fw-t2">SPEs em equiv.</text>'),
 ('<table class="tl" style="width:100%;font-size:11.5px;line-height:1.15"><thead><tr><th>parceria</th><th>origem (FR 1.2)</th><th>hoje</th><th>relação com Elie Horn</th><th>o que aconteceu</th></tr>',
  '<table class="tl socios" style="width:100%;font-size:9.5px;line-height:1.15;table-layout:fixed"><thead><tr><th style="width:9%">parceria</th><th style="width:25%">origem (FR 1.2)</th><th style="width:12%">hoje</th><th style="width:29%">relação com Elie Horn</th><th style="width:25%">o que aconteceu</th></tr>'),
 ('JV com a Cury Empreendimentos (família Cury), econômico SP e RJ; 48% antes do IPO', 'JV com a Cury Empreendimentos, econômico SP e RJ; 48% antes do IPO'),
 ('<b>Negócio + credor</b>: a Cyrela emprestou R$ 13,4 mi (CDI+1,72%) com penhor das cotas da SPE e das ações que o Silvio tem na SKR (FR, partes relacionadas)', '<b>Negócio + credor</b>: mútuo de R$ 13,4 mi (CDI+1,72%) com penhor de cotas da SPE e ações do Silvio'),
 ('<b>Família</b>: Ralph Horn é sobrinho de Elie e começou na Cyrela em 1987; o filho Dida Horn é CFO da Lavvi', '<b>Família</b>: Ralph Horn, sobrinho de Elie, entrou na Cyrela em 1987; o filho Dida Horn é CFO da Lavvi'),
 ('Ágio de R$ 528 mi; novo acordo de acionistas (4 anos) e saída do conselho em ago/26', 'Ágio de R$ 528 mi; novo acordo de acionistas (4 anos); saída do conselho ago/26'),
 # slide 18 (parcela): título a 38px numa linha (964px)
 ('<h2 class="head-xl">Demanda: a parcela tira o meio; o topo compra sem banco.</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">Demanda: a parcela tira o meio; o topo compra sem banco.</h2>'),
 # slide 22 (investidor): título numa linha a 38px (o original media 1092px > 1020 mesmo a 38px); o subtítulo longo do painel esquerdo (y=24) ficava por cima de "Quando valeu carregar imóvel" → desce para y=36, acima das faixas coloridas (y=40)
 ('<h2 class="head-xl">A ótica do investidor: aluguel mais valorização perde para a Selic.</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">Investidor: aluguel mais valorização perde para a Selic.</h2>'),
 ('<text x="830" y="24" class="gsub" style="font-size:10px" text-anchor="end">retorno 12m = apreciação (FipeZap SP) + yield de aluguel · vs Selic acumulada 12m · verde: imóvel ganha; vermelho: Selic ganha</text>', '<text x="830" y="36" class="gsub" style="font-size:10px" text-anchor="end">retorno 12m = apreciação (FipeZap SP) + yield de aluguel · vs Selic acumulada 12m · verde: imóvel ganha; vermelho: Selic ganha</text>'),
 # slide 23 (renda): título a 38px numa linha (o texto já foi trocado no 1º par desta lista)
 ('<h2 class="head-xl">A renda cresceu, mas o 3x veio do topo e do investidor.</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">A renda cresceu, mas o 3x veio do topo e do investidor.</h2>'),
 # slide 28 (INCC no MCMV): título numa linha a 38px (com "na obra" media 1135px; os cartões dizem onde o ativo corre)
 ('<h2 class="head-xl">INCC no MCMV: o passivo trava na assinatura, o ativo corre na obra.</h2>', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">INCC no MCMV: o passivo trava na assinatura, o ativo corre.</h2>'),
 # slide 43 (margem e régua): título sem <br> (923px a 46px); rótulos de fim de linha saem de dentro do gráfico (cruzavam as três séries em 2024-26) para a direita de x=830, com o viewBox alargado para 1040 (o rótulo "MCMV · 36,6% (2T26, trimestre)" é o mais longo)
 ('<h2 class="head-xl">A margem e a régua:<br>quem tem o INCC a favor.</h2>', '<h2 class="head-xl">A margem e a régua: quem tem o INCC a favor.</h2>'),
 ('<div class="viz" style="max-width:840px;margin:4px auto 0"><svg viewBox="0 0 900 252"><text x="70" y="24" class="gtit">Margem bruta por segmento e INCC, desde 2010</text>', '<div class="viz" style="max-width:900px;margin:4px auto 0"><svg viewBox="0 0 1040 252"><text x="70" y="24" class="gtit">Margem bruta por segmento e INCC, desde 2010</text>'),
 ('<line x1="830.0" y1="76.4" x2="763" y2="54.4" stroke="var(--s2)" stroke-width="1" opacity=".55"/>', ''),
 ('<text x="826" y="58.4" class="fw-t2" fill="var(--s2)" text-anchor="end">MCMV · 36,6% (2T26, trimestre)</text>', '<text x="838" y="78.4" class="fw-t2" fill="var(--s2)">MCMV · 36,6% (2T26, trimestre)</text>'),
 ('<line x1="830.0" y1="90.2" x2="763" y2="78.4" stroke="var(--s3)" stroke-width="1" opacity=".55"/>', ''),
 ('<text x="826" y="82.4" class="fw-t2" fill="var(--s3)" text-anchor="end">Living · 33,1%</text>', '<text x="838" y="92.2" class="fw-t2" fill="var(--s3)">Living · 33,1%</text>'),
 ('<line x1="830.0" y1="97.1" x2="763" y2="102.4" stroke="var(--s1)" stroke-width="1" opacity=".55"/>', '<line x1="830.0" y1="97.1" x2="836" y2="103" stroke="var(--s1)" stroke-width="1" opacity=".55"/>'),
 ('<text x="826" y="106.4" class="fw-t2" fill="var(--s1)" text-anchor="end">alto padrão · 31,3%</text>', '<text x="838" y="106.4" class="fw-t2" fill="var(--s1)">alto padrão · 31,3%</text>'),
 ('<line x1="830.0" y1="192.1" x2="763" y2="210.1" stroke="var(--muted)" stroke-width="1" opacity=".55"/>', ''),
 ('<text x="826" y="214.1" class="fw-t2" fill="var(--muted)" text-anchor="end">INCC-M 12m · 6,7%</text>', '<text x="838" y="196.1" class="fw-t2" fill="var(--muted)">INCC-M 12m · 6,7%</text>'),
]
...
_sm = _mslide("map", "Médio e alto padrão: o mercado desacelera, e a Cyrela vai junto.",
    ...
    f"SP ex-MCMV vende −21% em unidades; a Cyrela lança −{fmt(_cy_map_var, 0)}% e vende mais devagar (VSO 11%).", "O share de 2026 é parcial (jan-mai). Rio: sem série pública por padrão; o agregado (Ademi/Brain) ainda crescia no 1S26.", h2px=36)   # 22/09/26: mensagem no formato do autor; 19/09/26: 40px abria 2 linhas (1097px > 1020); 36px = 987px, uma linha; 23/09/26: "o share de 2026 é parcial" vai para o apoio (a mensagem media 992px a 18px e abria 2 linhas; agora 815px a 19px)
_sm = _sm.replace('<div class="sl-output" style="row-gap:3px;margin-top:6px">', '<div class="sl-output" style="row-gap:3px;margin-top:6px;align-items:flex-start;padding:7px 16px">', 1).replace('<span class="out-tag">', '<span class="out-tag" style="margin-top:3px">', 1)   # 22/09/26: tag alinhada à 1ª linha
...
_s48 = sl("parte 4 · onde estamos no ciclo", f"Cury: {fmt(_n2['pb_cury'], 1)}x book com ROE de {fmt(_n2['roe_cury'], 0)}%; a Vivaz, por analogia, ~{fmt(_n2['pb_viv'], 1)}x.".replace("ROE de", "ROE"),   # 22/09/26: o teto sai do título e da caixa verde; fica na tabela como cenário ("ROE 75%": com "de" o "~2,5x." caía na 2ª linha a 38px)
    ...
    nota=f"Fontes: Cury, planilha Fundamentos do RI: retorno operacional = lucro antes do financeiro LTM ÷ PL total médio; ROE = lucro dos controladores ÷ PL médio (12 meses); dívida líquida = empréstimos − caixa − títulos; terreno a prazo = credores por imóveis compromissados. B3, preços e ações em {_n2['mkt']['data']}. Vivaz: nota de segmentos do ITR (2T26), sem juros, IR e minoritários; ROE estimado = retorno operacional × {fmt(_n2['fator_ll'], 2)} (razão da Cury). P/B implícito = (ROE − g) ÷ (Ke − g), Ke 17% e g 4%. Cenário da tabela: lucro +{fmt(100 * _n2['g1'])}% a.a. por {_n2['n1']} anos reinvestindo (payout = 1 − g ÷ ROE), depois perpetuidade a g 4% — o ritmo em que a Vivaz alcança o lançado da Cury (R$ 8,1 bi) em dois anos. Vendas ÷ receita: Cury, vendas parte Cury; Vivaz, vendas × perímetro consolidado ({fmt(100 * _n2['cons'])}%) ÷ receita do segmento (a 100%, {fmt(_n2['vr_viv100'], 1)}x). Não é preço-alvo.").replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
...
_sbb = ('<div class="viz" style="margin-top:6px">' + SB["svg"] + '</div>' + SB["table"]
    + _obox(f'Goodwill de 2020 é R$ {fmt(_sb_["gw_tot"] / 1000, 1)} bi do PL; a fatia na P&P: R$ {fmt(_sb_["pp_book"])} mi no book, R$ {fmt(_sb_["pp_mkt"])} mi em bolsa.',
            f'Na perda de controle (2020) a Cyrela remensurou P&P e Lavvi a valor justo: R$ {fmt(_sb_["gw_2020"])} mi de goodwill, R$ {fmt(_sb_["gw_tot"])} mi hoje, sem amortização, só teste de valor em uso. {fmt(_sb_["roe_pp"], 0)}% de ROE na P&P viram {fmt(_sb_["ret_pp"], 0)}% sobre o book da Cyrela; na Lavvi, {fmt(_sb_["roe_lv"], 0)}% viram {fmt(_sb_["ret_lv"], 0)}%. Sem o goodwill o ROE sobe de {fmt(_sb_["roe_cy"], 1)}% para {fmt(_sb_["roe_ex_gw"], 1)}%; o risco é o inverso: impairment.'))   # 23/09/26: apoio em 2 linhas (antes 3), mesmos números
...
_ssb = ('<div class="viz" style="margin-top:6px">' + SS["svg"] + '</div>' + SS["table"]
    + _obox18(f'Para R$ {_ss["tp"]}, os dois bancos precisam do estoque girando mais: IBBA {fmt(_ss["vso_est_30"], 0)}%, BTG {fmt(_ss["btg_vso_30"], 0)}% ao ano; hoje 38%.',   # 23/09/26: 18px (879px a 19px abria 2 linhas; 833px a 18px)
...
final.append(_sss)
# --- do lucro bruto ao caixa: reconciliação de baixo para cima (23/09/26); dados de grafico_caixa_reconc.py
CXR = J("_caixa_reconc_frag.json"); _cx_ = CXR["num"]
_cxb = ('<div class="viz" style="margin-top:6px">' + CXR["svg"] + '</div>' + CXR["table"]
    + _obox(f'De R$ {fmt(_cx_["lb"] / 1000, 1)} bi de lucro bruto, obra e recebível levam {fmt((_cx_["cr"] + _cx_["obra"]) / 1000, 1)}; o terrenista financia {fmt(_cx_["terr"] / 1000, 1)}; sobram {fmt(_cx_["cia"] / 1000, 1)} bi.',
            f'O CPV reconhece o terreno na receita, mas o caixa saiu antes ou sai a prazo: terrenos devolvem R$ {fmt(_cx_["terr"] / 1000, 1)} bi no LTM ({fmt(_cx_["terr_2025"] / 1000, 1)} bi em 2025) e o caixa da obra supera o lucro bruto. JVs, SPEs e imobilizado ({fmt(_cx_["invest"] / 1000, 1)} bi) e minoritários ({fmt(_cx_["minor"] / 1000, 1)} bi) fecham a ponte; "outros" = R$ {fmt(_cx_["outros"], 0)} mi, sem viés desde 2015. Conversão no LTM: release = {fmt(_cx_["conv_lb"], 0)}% do lucro bruto e {fmt(_cx_["conv_ll"], 0)}% do lucro líquido, após três anos em zero ou negativa; erro médio: {fmt(_cx_["err_lb_abs_med"], 0)}% do lucro bruto desde 2015 e {fmt(_cx_["err_ll_abs_med"], 0)}% do lucro líquido desde 2019.'))   # 25/09/26: apoio em 3 linhas
_cxs = sl("parte 4 · lucro, caixa e payout", "Do lucro bruto ao caixa: obra e recebível levam R$ 2,9 bi.", _cxb,
    nota="Fontes: planilha de DFs do RI (DRE, balanço e fluxo de caixa trimestrais); CYREMod/notas dos ITR (obra, pronto e terrenos a custo; terrenos a pagar); balanço CVM (adiantamentos de clientes); provisão para distrato (1T20+); releases (geração de caixa operacional: 2020+ tabela ex-participações; 2013-19 texto, até 2016 somados os dividendos). Terrenos = Δ a custo − Δ a pagar: o custo reconhecido no CPV volta porque foi ou será pago em outra data. Financeiro em caixa = DRE + variação monetária não caixa + juros e outros itens pagos (DFC); investimentos = compra líquida de ativo permanente (DFC); minoritários = outras aplicações de financiamento (DFC). 'Outros' = resíduo: média +R$ 60 mi e desvio R$ 200 mi por ano desde 2015; 2013-14 têm definição diferente no release. Torre Cyrela Corporate by Pininfarina: R$ 431 mi reclassificados de estoque para imobilizado em 31/12/25 sem caixa (nota da DFP), tirados do Δ estoque e dos investimentos."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
final.insert(next(i for i, s in enumerate(final) if "Dividendo: um terço" in _h2(s)) + 1, _cxs)
# --- share por região metropolitana: Cury e Vivaz ÷ MCMV financiado na RMSP, RMRJ e Brasil (23/09/26); dados de grafico_share_rm.py
SRM = J("_share_rm_frag.json"); _sr = SRM["num"]
_srb = ('<div class="viz" style="margin-top:4px">' + SRM["svg"] + '</div>' + SRM["table"]
    + _obox(f'No Rio, a Cury é {fmt(_sr["c_rmrj"], 0)}% do MCMV financiado e a Vivaz {fmt(_sr["v_rmrj"], 0)}%; em São Paulo, {fmt(_sr["c_rmsp"], 0)}% e {fmt(_sr["v_rmsp"], 0)}% (12 meses).',
            f'O share estadual do slide anterior dilui as duas no interior e nos usados. Na região metropolitana, Cury e Vivaz somam {fmt(_sr["soma_rmsp_25"], 0)}% da RMSP e {fmt(_sr["soma_rmrj_25"], 0)}% da RMRJ em 2025. A Cury não abre unidades por praça: a estimativa por contagem de projetos e o teto pela Geoimóvel (total menos capital paulista, {fmt(_sr["c_geo_pct_25"], 0)}% em 2025) convergem em {fmt(_sr["c_teto_min"], 0)}% a {fmt(_sr["c_teto_max"], 0)}% da RMRJ desde 2023. No Brasil, {fmt(_sr["c_br"], 1)}% e {fmt(_sr["v_br"], 1)}%.'))
_srs = sl("parte 5 · atualização operacional · MCMV", "Por região: a Cury é um terço do Rio; a Vivaz, 14% de SP.", _srb,
    nota="Fontes: MCid, MCMV Financiado (FGTS/FS), unidades por município e mês de assinatura (ref. 24/07/26), somadas nos 39 municípios da RMSP, nos 22 da RMRJ e no país; inclui todas as faixas e imóveis usados. Vivaz: anexo 'Lançamentos' dos releases da Cyrela (4T17-4T25 e 2T26: empreendimento, região, unidades 100%, produto MCMV/CVA Faixas 2 e 3), sem as JVs Plano & Plano e Cury (marca Dez); 2026 estimado pelo ticket do RI; bate com a planilha do RI ano a ano. Cury: unidades lançadas da planilha do RI (100%) rateadas pela contagem de empreendimentos por praça dos releases (4T22+); releases, prévias e as 24 apresentações de resultados (3T20-2T26) só listam destaques, não a lista de lançamentos; cruzamento: Geoimóvel (Mercado Completo, cidade de São Paulo, unidades lançadas por grupo). 'Peso do Rio' = unidades no RJ ÷ (SP + RJ). LTM = 3T25-2T26."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
final.insert(next(i for i, s in enumerate(final) if "MCMV: o mercado segue" in _h2(s)) + 1, _srs)
# --- recebível: dias de contas a receber e contas a receber ÷ PL, Cyrela × Cury, e caixa que sai por cenário (23/09/26); dados de grafico_cr_dias.py
CRD = J("_cr_dias_frag.json"); _cd = CRD["num"]
_cdb = ('<div class="viz" style="margin-top:0">' + CRD["svg"] + '</div>' + CRD["table"] + CRD["table2"]
    + _obox(f'Só o mix contratado tira {fmt(_cd["cy_dias"] - _cd["dias31_base"], 0)} dias até 2031; a +30 pp, {fmt(_cd["cy_dias"] - _cd["dias31_p30"], 0)} dias e R$ {fmt(_cd["cx_p30_27_31"] - _cd["cx_base_27_31"], 1)} bi a mais de caixa.',
            f'Cyrela a {fmt(_cd["cy_dias"], 0)} dias, Cury a {fmt(_cd["cu_dias"], 0)}; Tenda {fmt(_cd["td_dias"], 0)} e P&amp;P {fmt(_cd["pp_dias"], 0)} como a Cury, Direcional {fmt(_cd["dr_dias"], 0)} e MRV {fmt(_cd["mrv_dias"], 0)} não; puros de MAP: Lavvi {fmt(_cd["lv_dias"], 0)}, Trisul {fmt(_cd["tr_dias"], 0)}, Even {fmt(_cd["ev_dias"], 0)}. Com a Vivaz a {fmt(_cd["dv"], 0)} dias, o resto roda a {fmt(_cd["dias_map"], 0)}. A Vivaz é {fmt(_cd["mix26"], 0)}% dos lançamentos e {fmt(_cd["sh_mcmv"], 0)}% da receita: a receita segue o lançamento com três anos de atraso, até 2028 os cenários empatam. Caixa = caixa antes do recebível no nível do LTM ({fmt(_cd["cx_ante_pct"], 0)}% da receita: release R$ {fmt(_cd["cia_ltm"] / 1000, 1)} bi + Δ recebível {fmt(_cd["dcr_ltm"] / 1000, 1)} bi) menos o Δ recebível de cada cenário; receita cresce {fmt(_cd["g"], 0)}% ao ano.'))   # 24/09/26: apoio em 3 linhas
_cds = sl("parte 4 · lucro, caixa e payout", f"Recebível: {fmt(_cd['cy_dias'], 0)} dias; o mix da Vivaz tira {fmt(_cd['cy_dias'] - _cd['dias31_base'], 0)} a {fmt(_cd['cy_dias'] - _cd['dias31_p30'], 0)} até 2031.", _cdb,
    nota=f"Fontes: Cyrela, planilha de DFs do RI (Economatica; contas a receber CP + LP líquido de AVP e distrato; receita desde 2010) e balanço CVM (PL consolidado); nota do ITR {_cd['nq']} (concluídas R$ {fmt(_cd['concl'], 0)} mi, em obra R$ {fmt(_cd['constr'], 0)} mi, bruto); segmentos (MCMV = Vivaz, 12 m). Cury: planilha Fundamentos do RI (contas a receber CP + LP, PL total, receita). Pares: ITR/DFP na CVM (receita 3.01), 2021+; MRV inclui Resia e Urba; Direcional, Riva; Tenda, Alea. Cenários: mix da Vivaz de 2027 em diante (base 32% do 1S26, +10, +20 e +30 pp); receita do ano = lançamentos de t−1 a t−4 a 15/35/35/15 (ciclo de 39 meses do anexo de entregues; 19% de Vivaz na receita de 2026 contra 17,7% observados); stake igual nos dois produtos (71-80%): mix a 100% vale a stake. Recebível ÷ PL da Cury: PL pequeno, não recebível longo."   # 24/09/26: nota em 4 linhas (era 5 com o painel 3); a fórmula dos dias já está no subtítulo do painel 1
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
final.insert(next(i for i, s in enumerate(final) if "Do lucro bruto ao caixa" in _h2(s)) + 1, _cds)
# --- projeção de caixa 2027-31 com as linhas da reconciliação, três cenários de lançamentos (24/09/26, v4: terreno em permuta; cenário 3 = lançar o que vende); dados de grafico_caixa_proj.py
CXP = J("_caixa_proj_frag.json"); _cp = CXP["num"]
_cpb = ('<div class="viz" style="margin-top:0">' + CXP["svg"] + '</div>' + CXP["table"]
    + _obox(f'2027 perto de zero nos três cenários; 2029: R$ {fmt(_cp["cx_ltm_29"] / 1000, 1)} bi lançando +5% e {fmt(_cp["cx_lstar_29"] / 1000, 1)} bi lançando o que vende.',
            f'Em 2027 a receita contratada sobe {fmt(100 * (_cp["rec_27"] / _cp["rec_ltm_27_base"] - 1), 0)}% e o recebível cresce R$ {fmt(-_cp["cr_ltm_27"] / 1000, 1)} bi em qualquer cenário. "Lançar o que vende" = R$ {fmt(_cp["lanc_lstar_27"] / 1000, 1)} bi, o nível que mantém o estoque à VSO de hoje: libera obra e recebível e custa receita de 2029 em diante ({fmt(_cp["rec_lstar_29"] / 1000, 1)} bi contra {fmt(_cp["rec_ltm_29"] / 1000, 1)}). Em 2023-25 a companhia gerou {fmt(_cp["cia_23_25"][0], 0)}, {fmt(_cp["cia_23_25"][1], 0)} e {fmt(_cp["cia_23_25"][2], 0)} mi; a projeção fica acima porque o lançamento para de crescer 40% ao ano.'))   # 24/09/26: apoio em 3 linhas (a frase das fórmulas repetia a coluna "premissa" da tabela)
_cps = sl("parte 4 · lucro, caixa e payout", f"Caixa 2029: R$ {fmt(_cp['cx_ltm_29'] / 1000, 1)} bi a +5%, {fmt(_cp['cx_lstar_29'] / 1000, 1)} bi lançando o que vende.", _cpb,
    nota=f"Fontes: reconciliação dos slides anteriores (planilha de DFs do RI, notas dos ITR, balanço CVM, releases; R$ mi); lançamentos do RI (VGV 100%). Cenários: (1) lançamentos +{fmt(_cp['g'], 0)}% ao ano sobre o LTM 2T26 (R$ {fmt(_cp['lanc_ltm'] / 1000, 1)} bi); (2) +5% sobre 2025 (R$ {fmt(_cp['lanc_25'] / 1000, 1)} bi); (3) lançar o que vende: L* = VSO × estoque ÷ (1 − VSO) do LTM (44% e R$ 17,2 bi → R$ {fmt(_cp['lanc_lstar_27'] / 1000, 1)} bi), depois +5%; (1) também com a Vivaz a +30 pp. Premissas na coluna da tabela: receita calibrada no LTM; demais linhas em % da receita na média 2023-LTM (investimentos sem a torre). Terreno em permuta: o saldo a pagar de R$ 3,2 bi (nota do ITR) rola com compras novas a prazo; se o terrenista parar de financiar, as parcelas (R$ 0,9 / 0,6 / 0,6 / 0,4 / 0,1 bi) saem sem reposição e o caixa de 2029 cai para R$ {fmt(_cp['cx_ltm_tc_29'] / 1000, 1)} bi; pagar o terreno novo à vista é neutro se vier com ~10% de desconto. Sem dividendos, recompra, venda de participações e juros além do financeiro da DRE."   # 24/09/26: nota em 4 linhas (as fórmulas linha a linha já estão na coluna "premissa")
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
final.insert(next(i for i, s in enumerate(final) if "Recebível:" in _h2(s)) + 1, _cps)
# --- conversão em caixa por fase e projetada (24/09/26); tabela e barras de grafico_caixa_reconc.py (agregados de _caixa_reconc_frag.json), projeção de _caixa_proj_frag.json
_cvb = ('<div class="viz" style="margin-top:4px">' + CXR["svg2"] + '</div>' + CXR["table2"]
    + _obox(f'2013-25: {fmt(_cx_["conv_lb_tot"], 0)}% do lucro bruto virou caixa; 2023-25: {fmt(_cx_["conv_lb_exp"], 0)}%; à frente, {fmt(_cx_["fwd_lb_perm"], 0)}% a {fmt(_cx_["fwd_lb_tc"], 0)}%.',
            f'A conversão caiu por fase: {fmt(_cx_["conv_lb_col"], 0)}% na colheita do legado (2013-16), {fmt(_cx_["conv_lb_rec"], 0)}% na reconstrução (2017-19, lucro perto de zero), {fmt(_cx_["conv_lb_ipo"], 0)}% com os IPOs e a retomada (2020-22) e {fmt(_cx_["conv_lb_exp"], 0)}% na expansão (2023-25); sobre o lucro líquido, {fmt(_cx_["conv_ll_tot"], 0)}% no total e {fmt(_cx_["conv_ll_exp"], 0)}% em 2023-25. O modelo do slide de projeção, lançando +5% ao ano, converte {fmt(_cx_["fwd_lb_perm"], 0)}% do lucro bruto e {fmt(_cx_["fwd_ll_perm"], 0)}% do lucro líquido lançando +5%, {fmt(_cx_["fwd_lb_tc"], 0)}% e {fmt(_cx_["fwd_ll_tc"], 0)}% lançando o que vende; com a Vivaz a +30 pp, {fmt(_cx_["fwd_lb_corte"], 0)}%. É o número a cobrar da companhia: a média histórica, não a expansão.'))
_cvs = sl("parte 4 · lucro, caixa e payout", f"Conversão: {fmt(_cx_['conv_lb_tot'], 0)}% do lucro bruto em 2013-25, {fmt(_cx_['fwd_lb_perm'], 0)}% a {fmt(_cx_['fwd_lb_tc'], 0)}% à frente.", _cvb,
    nota=f"Fontes: reconciliação do slide anterior (somas por fase; release = geração de caixa operacional da companhia; erro = release − reconciliado). Fases pela série de lançamentos e resultado: 2013-16 lançamentos caindo de R$ 6,6 bi para 2,9 bi e caixa do estoque entregue; 2017-19 prejuízos em 2017-18 e relançamento; 2020-22 IPOs de Cury, P&P e Lavvi e retomada; 2023-25 lançamentos de R$ 9,8 bi para 18,6 bi. Projeção 2027-31: modelo do slide de projeção de caixa (lançamentos +5% sobre o LTM, lançar o que vende, Vivaz +30 pp; terreno em permuta); lucro líquido projetado = lucro bruto × (lucro líquido ÷ lucro bruto do LTM, {fmt(100 * _cx_['ll'] / _cx_['lb'], 0)}%), porque o modelo não projeta a DRE abaixo do lucro bruto. 2017-19 ÷ lucro líquido fica fora da escala do gráfico."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:36px;margin-bottom:4px">', 1)   # 25/09/26: 38px abria 2 linhas ("frente." sozinho); 36px como nos outros slides
final.insert(next(i for i, s in enumerate(final) if "Do lucro bruto ao caixa" in _h2(s)) + 1, _cvs)
# --- valuation: lucro projetado, fluxo ao acionista e VPL por cenário (24/09/26, v4: Ke 17% e g 4% + sensibilidade em passos de 1 pp); dados de grafico_valuation.py
VLU = J("_valuation_frag.json"); _vl = VLU["num"]
_vlb = ('<div class="viz" style="margin-top:0">' + VLU["svg"] + '</div>' + VLU["table"] + VLU["table2"] + VLU["table3"]
    + _obox(f'A Ke de {fmt(_vl["ke"], 0)}%, o fluxo vale R$ {fmt(_vl["ps_ltm"], 0)} por ação lançando +5% e {fmt(_vl["ps_lstar"], 0)} lançando o que vende; a tela paga {fmt(_vl["px"], 2)}.',
            f'Lucro líquido de R$ {fmt(_vl["ll27"] / 1000, 1)} bi em 2027 e {fmt(_vl["ll29"] / 1000, 1)} bi em 2029 (P/L {fmt(_vl["pe27"], 1)}x e {fmt(_vl["pe29"], 1)}x), mas só {fmt(_vl["conv_ll_27_31"], 0)}% vira caixa em 2027-31. O VPL só encosta no preço lançando o que vende ou com a Vivaz a +30 pp (R$ {fmt(_vl["ps_lstar"], 0)} e {fmt(_vl["ps_ltmp30"], 0)}), ou se o Ke cair a 15% (R$ {fmt(_vl["sens"]["ltm@15"], 0)}). O mercado paga {fmt(_vl["pb"], 1)}x o book e {fmt(_vl["pe_ltm"], 1)}x o lucro do LTM: está precificando a conversão de 2013-19, não a de 2020-25.'))
_vls = sl("parte 4 · lucro, caixa e payout", f"Valuation: R$ {fmt(_vl['ps_ltm'], 0)} a {fmt(_vl['ps_ltmp30'], 0)} por ação a Ke de {fmt(_vl['ke'], 0)}%, contra {fmt(_vl['px'], 0)} na tela.", _vlb,
    nota=f"Fontes: modelo de caixa do slide anterior (receita e lucro bruto por cenário de lançamento, terreno em permuta); linhas abaixo do lucro bruto em % da receita na média de 2023-LTM, conferidas no LTM (lucro líquido R$ {fmt(_vl['ll_ltm'])} mi). Fluxo ao acionista = geração de caixa operacional projetada (já com financeiro, dividendos das JVs, investimentos e minoritários; sem dividendos pagos e recompra). VPL = fluxos de 2027-31 descontados a Ke {fmt(_vl['ke'], 0)}% (premissa do autor) do fim de 2026, mais perpetuidade sobre 2031 a g {fmt(_vl['g'], 0)}%, menos dívida líquida de 2T26 (R$ {fmt(_vl['dl'])} mi); {fmt(_vl['nsh'], 1)} mi de ações; preço CYRE3 de 18/09/26. A grade de sensibilidade é do cenário +5% s/ LTM. As participações em Cury, Lavvi e P&P entram pelos dividendos e pela equivalência, não a valor de mercado. Se o terrenista parar de financiar (R$ 3,2 bi a pagar sem reposição), o VPL do cenário base cai R$ {fmt(_vl['ps_ltm'] - _vl['ps_ltm_tc'], 0)} por ação." if "ps_ltm_tc" in _vl else f"Fontes: modelo de caixa do slide anterior (receita e lucro bruto por cenário de lançamento, terreno em permuta); linhas abaixo do lucro bruto em % da receita na média de 2023-LTM, conferidas no LTM (lucro líquido R$ {fmt(_vl['ll_ltm'])} mi). Fluxo ao acionista = geração de caixa operacional projetada (já com financeiro, dividendos das JVs, investimentos e minoritários; sem dividendos pagos e recompra). VPL = fluxos de 2027-31 descontados a Ke {fmt(_vl['ke'], 0)}% (premissa do autor) do fim de 2026, mais perpetuidade sobre 2031 a g {fmt(_vl['g'], 0)}%, menos dívida líquida de 2T26 (R$ {fmt(_vl['dl'])} mi); {fmt(_vl['nsh'], 1)} mi de ações; preço CYRE3 de 18/09/26. A grade de sensibilidade é do cenário +5% s/ LTM. As participações em Cury, Lavvi e P&P entram pelos dividendos e pela equivalência, não a valor de mercado."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
final.insert(next(i for i, s in enumerate(final) if "Caixa 2029" in _h2(s)) + 1, _vls)
# --- ROE DuPont histórico e projetado (24/09/26); dados de grafico_dupont_proj.py
DPP = J("_dupont_proj_frag.json"); _dp = DPP["num"]
_dpb = ('<div class="viz" style="margin-top:0">' + DPP["svg"] + '</div>' + DPP["table"]
    + _obox(f'ROE de {fmt(_dp["roe_ltm"], 0)}% para {fmt(_dp["roe27"], 0)}% em 2027 e {fmt(_dp["roe31"], 0)}% em 2031: margem parada em {fmt(_dp["mg27"], 0)}%, giro caindo, ROE no Ke.',
            f'Margem líquida constante por construção; o que cai é o giro sobre o PL, de {fmt(_dp["gp27"], 2)}x para {fmt(_dp["gp31"], 2)}x, porque o PL cresce mais que a receita: o modelo retém R$ {fmt(_dp["ret_27_31"] / 1000, 1)} bi de lucro em recebível, obra e SPEs em 2027-31 e o PL vai de R$ {fmt(_dp["pl0"] / 1000, 1)} bi para {fmt(_dp["pl31"] / 1000, 1)} bi. Lançando o que vende o ROE de 2031 é {fmt(_dp["roe31_lstar"], 0)}%; com a Vivaz a +30 pp, {fmt(_dp["roe31_p30"], 0)}%; crescendo sobre 2025, {fmt(_dp["roe31_2025"], 0)}%. ROE igual ao Ke de {fmt(_dp["ke"], 0)}% é o que o preço de 1,0x book já diz. O histórico: {fmt(_dp["roe_max"], 0)}% em {_dp["roe_max_ano"]} (marcação dos IPOs), {fmt(_dp["roe_2016"], 0)}% em 2016, {fmt(_dp["roe_2019"], 0)}% em 2019.'))
_dps = sl("parte 4 · lucro, caixa e payout", f"ROE: de {fmt(_dp['roe_ltm'], 0)}% para {fmt(_dp['roe31'], 0)}% em 2031 pelo giro, não pela margem.", _dpb,
    nota=f"Fontes: DFs/ITR (lucro atribuível e receita de 12 meses, ativo e PL médios dos controladores, 5 pontas), trimestral desde 2006; projeção = lucro líquido e caixa do modelo (slides anteriores), PL dos controladores crescendo pelo lucro retido com a Cyrela distribuindo o caixa que gera (hipótese consistente com o VPL); ROE = lucro ÷ PL médio. Giro sobre o PL (receita ÷ PL médio) porque o modelo não projeta o ativo; na tabela, o giro sobre o ativo projetado supõe alavancagem ativo ÷ PL constante em {fmt(_dp['alav_ltm'], 2)}x (2T26). Retendo tudo, o ROE de 2031 seria menor; com payout de 50% do lucro, maior, mas com a dívida líquida subindo (slide de valuation)."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
final.insert(next(i for i, s in enumerate(final) if "Valuation:" in _h2(s)) + 1, _dps)
# tag "Slide Novo" (estrela, caixa amarela, extremo direito do kick) nos slides criados em 18-19/09/26
_NOVOS = ("Lucro, caixa, dívida e payout", "Tecnisa: R$ 95 mi de equity", "Médio e alto padrão: o mercado desacelera", "MCMV: o mercado segue no recorde", "Banco a banco: a Caixa carrega", "Share no crédito habitacional sem FGTS", "Estouro no MAP", "Estouro no MCMV", "Cury × Vivaz", "tem venda direta dentro", "Receita por trimestre", "SP no mapa", "Retorno por vertical", "x book com ROE", "MAP × Vivaz por dentro", "Lei do Distrato: antes", "Retomadas em SP", "Risco de ciclo", "Dividendo: um terço", "metade do book", "Sell-side: o que Ita", "Do lucro bruto ao caixa", "Por região: a Cury", "Recebível:", "Caixa 2029", "Conversão em caixa:", "Valuation:", "ROE DuPont")
for _k, _s in enumerate(final):
...
<style>
  ...
  .deck table.tl.socios th, .deck table.tl.socios td { padding:3px 6px; }   /* slide das parcerias (23/09/26): cinco colunas de texto corrido; padding menor para nenhuma célula abrir 3 linhas */
