...
_cxb = ('<div class="viz" style="margin-top:6px">' + CXR["svg"] + '</div>' + CXR["table"]
    + _obox(f'De R$ {fmt(_cx_["lb"] / 1000, 1)} bi de lucro bruto, obra e recebível levam {fmt((_cx_["cr"] + _cx_["obra"]) / 1000, 1)}; o terrenista financia {fmt(_cx_["terr"] / 1000, 1)}; sobram {fmt(_cx_["cia"] / 1000, 1)} bi.',
            f'O CPV reconhece o terreno na receita, mas o caixa saiu antes ou sai a prazo: a linha de terrenos devolve R$ {fmt(_cx_["terr"] / 1000, 1)} bi no LTM ({fmt(_cx_["terr_2025"] / 1000, 1)} bi em 2025), por isso o caixa da obra supera o lucro bruto. JVs, SPEs e imobilizado ({fmt(_cx_["invest"] / 1000, 1)} bi) e minoritários ({fmt(_cx_["minor"] / 1000, 1)} bi) fecham a ponte; "outros" = R$ {fmt(_cx_["outros"], 0)} mi no LTM, sem viés desde 2015.'))
...
_cpb = ('<div class="viz" style="margin-top:0">' + CXP["svg"] + '</div>' + CXP["table"]
    + _obox(f'Permuta: R$ {fmt(_cp["cx_ltm_29"] / 1000, 1)} bi em 2029 lançando +5%; metade do terreno em caixa: {fmt(_cp["cx_ltm_tc_29"] / 1000, 1)} bi; corte: {fmt(_cp["cx_corte_29"] / 1000, 1)} bi.',
            f'Receita = {fmt(_cp["frec"], 0)}% dos lançamentos de t−1..t−4 (LTM abaixo porque 2025-26 vendeu mais devagar); obra = {fmt(_cp["beta"], 0)}% da média de 3 anos de lançamentos; demais linhas em % da receita; terreno em permuta (linha zero) ou metade do terreno do alto padrão pago em caixa (custo reconhecido no CPV menos caixa pago: R$ {fmt(_cp["cx_ltm_29"] / 1000 - _cp["cx_ltm_tc_29"] / 1000, 1)} bi por ano em 2029). Em 2027 o recebível cresce R$ {fmt(-_cp["cr_ltm_27"] / 1000, 1)} bi porque a receita já contratada sobe {fmt(100 * (_cp["rec_27"] / _cp["rec_ltm_27_base"] - 1), 0)}%; o corte de 30% libera obra e recebível e custa receita de 2029 em diante ({fmt(_cp["rec_corte_29"] / 1000, 1)} bi vs {fmt(_cp["rec_ltm_29"] / 1000, 1)}). Em 2023-25 gerou {fmt(_cp["cia_23_25"][0], 0)}, {fmt(_cp["cia_23_25"][1], 0)} e {fmt(_cp["cia_23_25"][2], 0)} mi: a projeção fica acima porque o lançamento para de crescer 40% ao ano, e a obra, de consumir caixa.'))
_cps = sl("parte 4 · lucro, caixa e payout", f"Caixa 2029: R$ {fmt(_cp['cx_ltm_29'] / 1000, 1)} bi com permuta, {fmt(_cp['cx_ltm_tc_29'] / 1000, 1)} bi pagando terreno.", _cpb,
    nota=f"Fontes: reconciliação do slide anterior (planilha de DFs do RI, notas dos ITR, balanço CVM, releases; R$ mi); lançamentos do RI (VGV 100%). Cenários: (1) lançamentos +{fmt(_cp['g'], 0)}% ao ano sobre o LTM 2T26 (R$ {fmt(_cp['lanc_ltm'] / 1000, 1)} bi); (2) +5% sobre 2025 (R$ {fmt(_cp['lanc_25'] / 1000, 1)} bi); (3) −30% em 2027 (entre o −15% do Itaú BBA para 2026 e o −50% de 2015-16), 2028 estável, depois +5%; (1) também com a Vivaz a +30 pp. Receita = {fmt(_cp['frec'], 0)}% × Σ lançamentos t−1..t−4, calibrado no LTM (2020-25 a razão foi ~100%); Δ estoque de obra = −Δ({fmt(_cp['beta'], 0)}% × média de 3 anos de lançamentos); Δ contas a receber pelos dias (Vivaz 160, resto {fmt(_cp['dias_map'], 0)}, mix 32% ou +30 pp); demais linhas em % da receita (2023-LTM); terrenos: permuta = 0 (2019-22 a linha foi −1% a −2% dos lançamentos; 2023-LTM, positiva pelo terrenista) ou em caixa = custo do terreno no CPV (18% da receita MAP) menos caixa pago (18% do VGV MAP lançado: metade no lançamento, metade em dois anos; 2013, compra à vista: −12% dos lançamentos). Sem dividendos, recompra, venda de participações e juros além do financeiro da DRE."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
