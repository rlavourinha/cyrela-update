...
# --- projeção de caixa 2027-31 com as linhas da reconciliação (24/09/26); dados de grafico_caixa_proj.py
CXP = J("_caixa_proj_frag.json"); _cp = CXP["num"]
_cpb = ('<div class="viz" style="margin-top:0">' + CXP["svg"] + '</div>' + CXP["table"]
    + _obox(f'Obra e terreno neutros: R$ {fmt(_cp["B_29"] / 1000, 1)} bi por ano; o LTM de {fmt(_cp["ltm_cia"] / 1000, 1)} bi tinha {fmt(_cp["ltm_terr"] / 1000, 1)} bi de terrenista.',
            f'Cada linha no nível de 2023-LTM em % da receita (margem bruta {fmt(_cp["mg"], 0)}%, SG&A {fmt(-_cp["sga"], 0)}%), receita +{fmt(_cp["g"], 0)}% ao ano e o recebível pelo cenário de dias do slide anterior. O que decide é o capital de giro de obra e terreno: como 2023-LTM (terrenista financiando {fmt(_cp["terrA"], 0)}% da receita), R$ {fmt(_cp["A_29"] / 1000, 1)} bi em 2029; como 2019-22, R$ {fmt(_cp["C_29"] / 1000, 1)} bi. A Vivaz a +30 pp acrescenta R$ {fmt((_cp["B30_31"] - _cp["B_31"]) / 1000, 1)} bi por ano no fim do período. Em 2023-25 gerou {fmt(_cp["cia_23_25"][0], 0)}, {fmt(_cp["cia_23_25"][1], 0)} e {fmt(_cp["cia_23_25"][2], 0)} mi: a projeção só fica acima porque supõe o ciclo de obra parando de consumir caixa.'))   # 24/09/26: apoio em 3 linhas
_cps = sl("parte 4 · lucro, caixa e payout", f"Caixa projetado: R$ {fmt(_cp['B_29'] / 1000, 1)} bi ao ano se obra e terreno zerarem.", _cpb,
    nota=f"Fontes: reconciliação do slide anterior (planilha de DFs do RI, notas dos ITR, balanço CVM, releases; R$ mi). Premissas: receita = LTM 2T26 × (1 + {fmt(_cp['g'], 0)}%)^n; cada linha em % da receita na média de 2023-LTM (lucro bruto {fmt(_cp['mg'], 1)}%, SG&A {fmt(_cp['sga'], 1)}%, impostos, financeiro, outras, dividendos de JVs, investimentos e minoritários); Δ contas a receber = variação do recebível pelo cenário de dias (Vivaz a 160, resto a 258, mix de lançamentos base 32% ou +30 pp com defasagem 15/35/35/15). Regimes de capital de giro: A = Δ estoque de obra e terrenos como 2023-LTM ({fmt(_cp['obraA'], 1)}% e {fmt(_cp['terrA'], 1)}% da receita); B = neutros; C = como 2019-22 ({fmt(_cp['obraC'], 1)}% e {fmt(_cp['terrC'], 1)}%). Não inclui dividendos, recompra, venda de participações nem juros sobre a dívida além do financeiro da DRE. Ponte de 2029 no regime B."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
