...
_cpb = ('<div class="viz" style="margin-top:0">' + CXP["svg"] + '</div>' + CXP["table"]
    + _obox(f'2027 perto de zero nos três cenários; 2029: R$ {fmt(_cp["cx_ltm_29"] / 1000, 1)} bi lançando +5% e {fmt(_cp["cx_lstar_29"] / 1000, 1)} bi lançando o que vende.',
            f'Receita = {fmt(_cp["frec"], 0)}% dos lançamentos de t−1 a t−4 (15/35/35/15); estoque de obra = {fmt(_cp["beta"], 0)}% da média de três anos de lançamentos; recebível pelos dias; demais linhas em % da receita (2023-LTM); terreno em permuta, com o saldo a pagar rolando. Em 2027 a receita contratada sobe {fmt(100 * (_cp["rec_27"] / _cp["rec_ltm_27_base"] - 1), 0)}% e o recebível cresce R$ {fmt(-_cp["cr_ltm_27"] / 1000, 1)} bi em qualquer cenário. "Lançar o que vende" = R$ {fmt(_cp["lanc_lstar_27"] / 1000, 1)} bi, o nível que mantém o estoque à VSO de hoje: libera obra e recebível e custa receita de 2029 em diante ({fmt(_cp["rec_lstar_29"] / 1000, 1)} bi contra {fmt(_cp["rec_ltm_29"] / 1000, 1)}). Em 2023-25 a companhia gerou {fmt(_cp["cia_23_25"][0], 0)}, {fmt(_cp["cia_23_25"][1], 0)} e {fmt(_cp["cia_23_25"][2], 0)} mi; a projeção fica acima porque o lançamento para de crescer 40% ao ano.'))
_cps = sl("parte 4 · lucro, caixa e payout", f"Caixa 2029: R$ {fmt(_cp['cx_ltm_29'] / 1000, 1)} bi a +5%, {fmt(_cp['cx_lstar_29'] / 1000, 1)} bi lançando o que vende.", _cpb,
    nota=f"Fontes: reconciliação dos slides anteriores (planilha de DFs do RI, notas dos ITR, balanço CVM, releases; R$ mi); lançamentos do RI (VGV 100%). Cenários: (1) lançamentos +{fmt(_cp['g'], 0)}% ao ano sobre o LTM 2T26 (R$ {fmt(_cp['lanc_ltm'] / 1000, 1)} bi); (2) +5% sobre 2025 (R$ {fmt(_cp['lanc_25'] / 1000, 1)} bi); (3) lançar o que vende: L* = VSO × estoque ÷ (1 − VSO) do LTM (44% e R$ 17,2 bi → R$ {fmt(_cp['lanc_lstar_27'] / 1000, 1)} bi), depois +5%; (1) também com a Vivaz a +30 pp. Receita = {fmt(_cp['frec'], 0)}% × Σ lançamentos t−1..t−4 (15/35/35/15), calibrado no LTM; Δ estoque de obra = −Δ({fmt(_cp['beta'], 0)}% × média de 3 anos); Δ contas a receber pelos dias (Vivaz 160, resto {fmt(_cp['dias_map'], 0)}); lucro bruto {fmt(_cp['mg'], 1)}%, SG&A {fmt(_cp['sga'], 1)}%, impostos, financeiro, outras, dividendos de JVs, investimentos (sem a torre) e minoritários em % da receita (2023-LTM). Terreno em permuta: o saldo a pagar de R$ 3,2 bi (nota do ITR) rola com compras novas a prazo; se o terrenista parar de financiar, as parcelas (R$ 0,9 / 0,6 / 0,6 / 0,4 / 0,1 bi) saem sem reposição e o caixa de 2029 cai para R$ {fmt(_cp['cx_ltm_tc_29'] / 1000, 1)} bi; pagar o terreno novo à vista é neutro se vier com ~10% de desconto. Sem dividendos, recompra, venda de participações e juros além do financeiro da DRE."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
