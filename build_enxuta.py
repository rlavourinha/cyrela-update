...
_cds = sl("parte 4 · lucro, caixa e payout", f"Recebível: {fmt(_cd['cy_dias'], 0)} dias na Cyrela, {fmt(_cd['cu_dias'], 0)} na Cury: R$ {fmt(_cd['lib_a'] / 1000, 1)} bi presos.", _cdb,
    nota=f"Fontes: Cyrela, planilha de DFs do RI (Economatica; contas a receber de clientes CP + LP líquido de AVP e provisão para distrato; receita trimestral, consolidado, desde 2010) e balanço CVM (PL consolidado); nota de contas a receber do ITR {_cd['nq']} (unidades concluídas R$ {fmt(_cd['concl'], 0)} mi e em construção R$ {fmt(_cd['constr'], 0)} mi, bruto); receita por segmento da nota de segmentos (MCMV = Vivaz, 12 meses). Cury: planilha Fundamentos do RI (balanço: contas a receber CP + LP, PL total; DRE: receita). Dias = contas a receber ÷ receita de 12 meses × 365. Cenários: (a) recebível da Cyrela = receita 12 m × dias da Cury; (b) MCMV nos dias da Cury e MAP nos dias implícitos de hoje, com o MCMV subindo de {fmt(_cd['sh_mcmv'], 0)}% para 30% e 40% da receita, mesma receita total. O recebível ÷ PL da Cury é alto porque o PL é pequeno (distribui quase todo o lucro), não porque o recebível é longo."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
