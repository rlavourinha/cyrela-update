...
body += output('ROE por alavancagem operacional: lucro 12m de R$ ' + fmt(ll12[_i22], 1) + ' bi para R$ ' + fmt(_llL, 1) + ' bi desde 2022, PL +' + fmt(_plg, 0) + '%, dívida líquida em ' + fmt(_plL, 0) + '% do PL. A empresa cresceu sem pedir capital.', 'O balanço não inchou: o MCMV gira com pouco capital (a CEF financia a obra sobre o vendido) e o terreno entra a prazo e em permuta. O dividendo de R$ ' + fmt(_sum4(DIV_Q, qs_f[-1], False) / 1000, 1) + ' bi saiu dessa folga, não do caixa operacional de R$ ' + fmt(_gcL, 1) + ' bi.'
              ).replace('<div class="sl-output">', '<div class="sl-output" style="align-items:flex-start;margin-top:6px;row-gap:4px;padding:7px 16px">', 1).replace('<span class="out-tag">', '<span class="out-tag" style="margin-top:3px">', 1).replace('<span class="out-msg">', '<span class="out-msg" style="flex:1 1 700px;line-height:1.2">', 1)   # mensagem longa (2 linhas ao lado da tag, não em linha própria): tag alinhada à 1ª linha (flex-start + 3px), menos respiro entre cartões e caixa e entre as linhas da caixa; 715 -> 706px
...
_s8 = re.sub(r'<div class="fwgrid" style="margin-top:16px">\s*<div class="fwcard map">.*?</div>\s*</div>\s*(?=</div>)',
    ...
    '<p>Terreno <b>10% do VGV</b> · margem bruta <b>~32%</b> (abaixo dos 36% do 2T26, por conservadorismo) · VSO <b>~25% na largada</b>, ~30%/tri, esgota no mês 30 · <b>90% na obra</b> (CEF, medição sobre o vendido) · <b>10% na entrega</b> · tracejada: terreno <b>100% em permuta</b>.</p></div></div>', _s8, count=1, flags=re.S)
...
_s8_add = ('<div class="fwgrid" style="grid-template-columns:1.15fr 1fr;margin-top:6px;gap:10px;align-items:start">' + re.sub(r' style="[^"]*"', ' style="margin-top:0"', _viz41, count=1)
    ...
    + '<div class="viz" style="margin-top:6px"><table class="tl" style="width:100%;font-size:10px;line-height:1.1;border:2px solid var(--s2);border-collapse:collapse;text-align:center"><thead>'
       '<tr style="background:var(--s2);color:#fff"><th rowspan="2" style="text-align:left;padding:2px 6px;vertical-align:middle;width:20%">retorno consolidado = w × Vivaz + (1 − w) × médio e alto padrão</th><th colspan="4" style="padding:2px 6px;border-left:1px solid rgba(255,255,255,.5)">Vivaz a 40% (hoje)</th><th colspan="4" style="padding:2px 6px;border-left:1px solid rgba(255,255,255,.5)">Vivaz a 30% (margem cai a 30%)</th></tr>'
       '<tr style="background:var(--s2);color:#fff">' + ''.join(f'<th style="padding:2px 4px{";border-left:1px solid rgba(255,255,255,.5)" if k == 0 else ""}">MAP {lab}</th>' for _ in (0, 1) for k, lab in enumerate(("23% (hoje)", "15% (2015-16)", "8% (2019)", "3,6% (fundo 2018)"))) + '</tr></thead><tbody>'
       + ''.join('<tr' + (' style="background:rgba(70,110,170,.12);font-weight:600"' if w == 0.2 else '') + f'><td style="text-align:left;padding:2px 6px">Vivaz com {fmt(100 * w, 0)}% do capital{" (≈ hoje)" if w == 0.2 else ""}</td>' + ''.join(f'<td style="padding:2px 5px{";border-left:1px solid var(--s2)" if k == 0 else ""}">{fmt(w * rv + (1 - w) * rm, 1)}%</td>' for rv in (40, 30) for k, rm in enumerate((23, 15, 8, 3.6))) + '</tr>' for w in (0.2, 0.33, 0.5))
    ...
    + output('O ROE continua subindo se a Vivaz crescer mais que o médio e alto padrão: mix, não ciclo, a 40% contra 23% de retorno.', 'A Vivaz é 18% da receita e 26% do lançado em consolidação; chegar a um terço com o alto padrão parado vale ~2,5 p.p. — se a margem da Vivaz sobreviver ao INCC e à Caixa.')
      .replace('<span class="out-msg">', '<span class="out-msg" style="flex:1 1 560px">', 1).replace('<span class="out-sub">', '<span class="out-sub" style="flex:1 1 250px">', 1))   # mensagem e subtítulo lado a lado (sem isso a caixa verde abre em três linhas de flex, 151px)
...
_sm = _mslide("map", "Médio e alto padrão: o mercado desacelera, e a Cyrela vai junto.",
    ...
    f'<b>Share em São Paulo capital</b> (Geoimóvel, VGV · unidades): {fmt(_n["geo_2019"][0], 1)}% · {fmt(_n["geo_2019"][1], 1)}% em 2019, {fmt(_n["geo_2025"][0], 1)}% · {fmt(_n["geo_2025"][1], 1)}% em 2025, {fmt(_n["geo_ult"][1], 1)}% · {fmt(_n["geo_ult"][2], 1)}% em jan-mai/26: a Cyrela ganha fatia num mercado que encolhe. <b>SP fora do MCMV</b>: vendas −21% em unidades e −11% em VGV em 12 meses. <b>Brasil</b> (ABRAINC): lançamentos −{fmt(100 * (1 - _n["map_lanc_ult"][1] / _n["map_lanc_pico"][1]))}% e vendas −{fmt(100 * (1 - _n["map_vend_ult"] / _n["map_vend_pico"][1]))}% desde o pico.',   # 19/09/26: cartão enxuto (mesmos números, menos palavras: o painel já mostra a série do share)
    "A queda no médio e alto padrão é o mercado, não a Cyrela: o share em São Paulo sobe.", "Rio: sem série pública por padrão; o agregado (Ademi/Brain) ainda crescia no 1S26.", h2px=36)   # 19/09/26: mensagem encurtada (a versão longa media 866px e abria 2 linhas mesmo a 19px)   # 19/09/26: 40px abria 2 linhas (1097px > 1020); 36px = 987px, uma linha
...
_scv = sl("parte 5 · atualização operacional · MCMV", "Cury × Vivaz: a Cury vende mais rápido, com ticket maior.",
    ...
    + _obox(f'Na mesma idade a Cury vende mais: {_cv["saf8"]} aos {_cv["idade8"]} meses, {fmt(_cv["cury8"], 0)}% contra {fmt(_cv["vivaz8"], 0)}%; VSO {fmt(_cv["vso_cury_med"], 0)}% contra {fmt(_cv["vso_vivaz_med"], 0)}%.',
...
_s48 = sl("parte 4 · onde estamos no ciclo", f"Cury: {fmt(_n2['pb_cury'], 1)}x book com ROE de {fmt(_n2['roe_cury'], 0)}%; a Vivaz valeria {fmt(_n2['pb_viv'], 1)}x a {fmt(_n2['pb_viv_top'], 1)}x.",
...
_svd = sl("parte 5 · atualização operacional · MCMV", f"Cury: VSO de {fmt(_d['vso'], 0)}% com venda direta; sem ela, {fmt(_d['vsox2'], 0)}% ou {fmt(_d['vsox'], 0)}%.",
    ...
    + _obox(f'Sem a venda direta, a VSO da Cury cai de {fmt(_d["vso_m4"], 0)}% para {fmt(_d["vsox2_m4"], 0)}% ou {fmt(_d["vsox_m4"], 0)}%, perto da Vivaz ({fmt(_d["vz_m4"], 0)}%).',
            f'Duas leituras: só tirando a venda direta do vendido, {fmt(_d["vsox2_m4"], 0)}%; devolvendo a carteira ao estoque, {fmt(_d["vsox_m4"], 0)}%. A velocidade extra é crédito da própria Cury: carteira de venda direta de R$ {fmt(_d["vd0"], 1)} bi para {fmt(_d["vd"], 1)} bi desde {_d["x0"]} ({fmt(_d["d0"], 0)}% para {fmt(_d["d"], 0)}% das vendas), mais R$ {fmt(_d["ps"], 1)} bi de pró-soluto.'),
...
_db = ('<div class="viz" style="margin-top:6px">' + DF["svg"] + '</div>' + _cards
    + _obox(f'O pior ficou antes da lei: {fmt(_dn["pct_0118"], 0)}% do recebível provisionado na abertura de 2018, {fmt(_dn["pct_now"], 0)}% hoje.',
            f'Saldo de R$ {fmt(_dn["cy_saldo_0118"], 0)} mi (jan/18) para {fmt(_dn["cy_saldo"], 0)} mi ({_dn["q"]}) com a venda dobrando; hoje é reciclagem, não perda: R$ {fmt(_dn["cy_adic_ltm"], 0)} mi de adições e {fmt(-_dn["cy_rev_ltm"], 0)} mi de reversões em 12 meses. Follow-up: Temas 1.464-1.466 (teto de 50% com afetação; a Vivaz tem afetação em todo MCMV).'))
_dsl = sl("anexo · follow-up · Lei do Distrato", "Lei do Distrato: antes, a lei e o que o STJ já testou.", _db, cls="anexo",
    nota="Fontes: Lei 13.786/2018 (arts. 35-A, 43-A e 67-A da Lei 4.591/1964); STJ, Súmula 543 (2015) e Tema 938 (2016); AgInt no REsp 2.159.971/SP (4ª Turma, rel. min. Antonio Carlos Ferreira, j. 15/06/2026, DJEN 18/06/2026) e EDcl no AgInt no REsp 2.221.464/SP (4ª Turma, rel. min. Raul Araújo, set/2026); 2ª Seção, Temas repetitivos 1.464, 1.465 e 1.466 (afetados, sem julgamento de mérito). Cyrela: DFP 2015-2017 (nota de provisões para riscos, item 'distratos de clientes'); DFP 2018 e 2019 e ITR 1T19-3T19 (nota de contas a receber: provisão para distrato do CPC 47/Ofício CVM 02/2018, saldo de abertura em 01/01/2018, adições e reversões); ITR 1T20-2T26 (idem, trimestral). Provisão ÷ contas a receber de vendas apropriado (consolidado). A companhia não divulga distratos em VGV, só vendas líquidas; entre 1T18 e 3T18 a provisão ainda não existia nessa forma (linha tracejada). Não há lei nova em 2025-26: a de 2018 está sendo interpretada."
...
_rb = ('<div class="viz" style="margin-top:6px">' + RT["svg"] + '</div>' + RT["table"] + '<div class="viz" style="margin-top:8px">' + CX["svg"] + '</div>'
    + _obox(f'Retomada em SP: de {fmt(_ra["2020"]["shr"], 1)}% para {fmt(_ra["2025"]["shr"], 1)}% das compras residenciais, e {fmt(_ra["2025"]["p350"], 0)}% delas até R$ 350 mil.',
            f'Média de 12 meses de {fmt(_rn["ret12_min"], 0)} retomadas por mês (mar/21) para {fmt(_rn["ret12"], 0)} (jul/26); os leilões sobem menos ({fmt(_rn["arr12"], 0)} por mês) e o estoque acumula no banco: na Caixa, os imóveis retomados foram de R$ {fmt(_cx["rec_min"] / 1000, 1)} bi (jun/22) a {fmt(_cx["rec_now"] / 1000, 1)} bi (jun/26), com R$ {fmt(_cx["desp_2025"] / 1000, 1)} bi de despesa em 2025. Para a incorporadora, o sinal pega no pró-soluto, na venda direta e na demanda do MCMV.'))
_rsl = sl("anexo · follow-up · retomadas em SP", "Retomadas em SP: 4x desde 2019, e cada vez mais baratas.", _rb, cls="anexo",
    nota=f"Fonte: Prefeitura de São Paulo, Secretaria da Fazenda, guias de ITBI pagas (dados abertos), jan/2019 a jul/2026, pelo mês da guia. Retomada = natureza '17. Resolução da alienação fiduciária por inadimplemento' (o credor fiduciário consolida a propriedade e recolhe o ITBI; Lei 9.514/1997, art. 26); leilão = natureza '4. Arrematação em leilão ou hasta pública' (inclui leilões judiciais, não só de imóveis retomados). Residencial = uso IPTU 10 (residência) e 20 (apartamento). Faixa e valor médio pela base de cálculo (valor venal de referência), porque na retomada não há preço de venda. Denominador da razão: guias de compra e venda residenciais com 100% transmitido. Só o município de São Paulo. Caixa: demonstrações contábeis semestrais (RI): 'ativos não financeiros mantidos para venda, recebidos' (bruto; até jun/20, 'imóveis adjudicados/arrematados' da nota antiga, pontilhado), provisão para desvalorização e a despesa 'imóveis adjudicados/arrematados' (1S22-1S26)."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
...
_rcb = ('<div class="viz" style="margin-top:6px">' + RC["svg"] + '</div>' + RC["table"]
    + _obox(f'A obra se paga; o que trava é o pronto: R$ {fmt(_rc["pronto_fim_atual"] / 1000, 1)} bi a {fmt(_rc["pronto_fim_2016"] / 1000, 1)} bi sem vender em 3 anos.',
            f'O estoque MAP (R$ {fmt(_rc["e_map"] / 1000, 1)} bi a mercado, 100%) tem R$ {fmt(_rc["custo_map"] / 1000, 1)} bi de obra a pagar; com {fmt(_rc["vso_atual"], 0)}%/tri de VSO de estoque, o que vende cobre a obra (30% na obra, 70% nas chaves), mas R$ {fmt(_rc["novo_pronto_atual"] / 1000, 1)} bi chegam prontos sem dono e um quarto do que está em obra segue sem vender no ano 3. Capital já investido no estoque, a custo: R$ {fmt(_rc["custo_investido"] / 1000, 1)} bi.'))
...
_dvs = sl("parte 4 · lucro, caixa e payout", "Dividendo: um terço veio da venda das sócias, não do caixa.", _dvb,
    nota=f"Fontes: B3 (proventos por ação com data ex, × ações ex-tesouraria: DFs e CYREMod até 2019, ITR desde 2020), por ano da data ex, conferido com a linha '(+) dividendos' da tabela de geração de caixa dos releases (pago no trimestre; iguais por ano de 2019 a 2025, o 12m difere só pelo trimestre de pagamento); lucro atribuível aos controladores (planilha de DFs do RI); releases, tabela de geração de caixa: linha '(+) aquisição/venda de participação societária' (entrada = venda; 2020 IPOs de Cury, P&P e Lavvi; 2022, 2024 e 2025 ações da Cury) e 'geração/consumo de caixa operacional' (definição da companhia, exclui recompra e participações; publicada desde 2020). Ganho contábil: 2020 R$ 1.335 mi (marcação a valor justo na perda de controle + secundária, ledger de não recorrentes), 2022 R$ 139 mi (release 3T22), 2024 ~R$ 135 mi (estimado, 2T24 e 3T24), 2025 R$ 240 mi (3T25). 2017-18 com prejuízo: sem payout. 12m = jul/25 a jun/26; yield sobre R$ {fmt(_dv['preco'], 2)} ({_dv['preco_dt'][8:]}/{_dv['preco_dt'][5:7]}/{_dv['preco_dt'][2:4]}), provento ajustado pela bonificação de jan/26."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
...
_NOVOS = ("Lucro, caixa, dívida e payout", "Tecnisa: R$ 95 mi de equity", "Médio e alto padrão: o mercado desacelera", "MCMV: o mercado segue no recorde", "Banco a banco: a Caixa carrega", "Share no crédito habitacional sem FGTS", "Estouro no MAP", "Estouro no MCMV", "Cury × Vivaz", "tem venda direta dentro", "Receita por trimestre", "SP no mapa", "Retorno por vertical", "x book com ROE", "MAP × Vivaz por dentro", "Lei do Distrato: antes", "Retomadas em SP", "Risco de ciclo", "Dividendo: um terço")
for _k, _s in enumerate(final):
