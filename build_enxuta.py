...
_svd = sl("parte 5 · atualização operacional · MCMV", f"Cury: a VSO de {fmt(_d['vso'], 0)}% tem venda direta dentro; sem ela, {fmt(_d['vsox'], 0)}%.",
    ...
    + _obox(f'Sem a venda direta, a VSO da Cury cai de {fmt(_d["vso_m4"], 0)}% para {fmt(_d["vsox_m4"], 0)}% na média de um ano, o patamar da Vivaz ({fmt(_d["vz_m4"], 0)}%).',
...
_NOVOS = ("Lucro, caixa, dívida e payout", "Tecnisa: R$ 95 mi de equity", "Médio e alto padrão: o mercado desacelera", "MCMV: o mercado segue no recorde", "Banco a banco: a Caixa carrega", "Share no crédito habitacional sem FGTS", "Estouro no MAP", "Estouro no MCMV", "Cury × Vivaz", "tem venda direta dentro", "Receita por trimestre", "SP no mapa", "Retorno por vertical", "x book com ROE", "MAP × Vivaz por dentro")
for _k, _s in enumerate(final):
