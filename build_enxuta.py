...
_cpb = ('<div class="viz" style="margin-top:4px">' + CXP["svg"] + '</div>' + CXP["table"]
    + _obox(f'Obra e terreno neutros: R$ {fmt(_cp["B_29"] / 1000, 1)} bi por ano; o LTM de {fmt(_cp["ltm_cia"] / 1000, 1)} bi tinha {fmt(_cp["ltm_terr"] / 1000, 1)} bi de terrenista.',
            f'Cada linha no nível de 2023-LTM em % da receita (margem bruta {fmt(_cp["mg"], 0)}%, SG&A {fmt(-_cp["sga"], 0)}%), receita +{fmt(_cp["g"], 0)}% ao ano e o recebível pelo cenário de dias do slide anterior. O que decide o número é o capital de giro de obra e terreno: repetindo 2023-LTM (terrenista financiando {fmt(_cp["terrA"], 0)}% da receita), R$ {fmt(_cp["A_29"] / 1000, 1)} bi em 2029; como em 2019-22, R$ {fmt(_cp["C_29"] / 1000, 1)} bi. A Vivaz a +30 pp acrescenta R$ {fmt((_cp["B30_31"] - _cp["B_31"]) / 1000, 1)} bi por ano no fim do período. Em 2023-25 a companhia gerou {fmt(_cp["cia_23_25"][0], 0)}, {fmt(_cp["cia_23_25"][1], 0)} e {fmt(_cp["cia_23_25"][2], 0)} mi: a projeção só fica acima disso porque supõe o ciclo de obra parando de consumir caixa.'))
