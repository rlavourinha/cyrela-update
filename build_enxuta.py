...
_ssb = ('<div class="viz" style="margin-top:6px">' + SS["svg"] + '</div>' + SS["table"]
    + _obox(f'Para o alvo de R$ {_ss["tp"]} o Itaú BBA precisa da VSO de estoque de {fmt(_ss["vso_est_26"], 0)}% para {fmt(_ss["vso_est_30"], 0)}% ao ano.',
            f'Lançamento cai {fmt(-_ss["lanc_var"], 0)}% em 2026 e o estoque do alto padrão fica parado em R$ 11 bi (o risco do slide 59), resolvido com giro; distratos em {fmt(_ss["cancel_26"], 0)}% das vendas brutas e dividendo zero em 2026. Na Cury, o alvo de R$ {_ss["tp_cu"]} não modela a venda direta: o recebível de clientes dobra até 2030 (R$ {fmt(_ss["cu_cr_25"], 1)} para {fmt(_ss["cu_cr_30"], 1)} bi) como se fosse repasse normal.'))
