...
_vls = sl("parte 4 · lucro, caixa e payout", f"Valuation: R$ {fmt(_vl['ps_ltm'], 0)} a {fmt(_vl['ps_corte'], 0)} por ação a Ke de {fmt(_vl['ke'], 0)}%, contra {fmt(_vl['px'], 0)} na tela.", _vlb,
    nota=f"Fontes: modelo de caixa do slide anterior (receita e lucro bruto por cenário de lançamento e regime de terreno); linhas abaixo do lucro bruto em % da receita na média de 2023-LTM (SG&A {fmt(_vl['pr']['sga'], 1)}%, financeiro {fmt(_vl['pr']['fin'], 1)}%, equivalência {fmt(_vl['pr']['equiv'], 1)}%, IR corrente {fmt(_vl['pr']['ir_corr'], 1)}%, diferido {fmt(_vl['pr']['ir_dif'], 1)}%, minoritários {fmt(_vl['pr']['minor'], 1)}%), conferidas no LTM (lucro líquido R$ {fmt(_vl['ll_ltm'])} mi). Fluxo ao acionista = geração de caixa operacional projetada (já com financeiro, dividendos das JVs, investimentos e minoritários; sem dividendos pagos e recompra). VPL = fluxos de 2027-31 descontados a Ke {fmt(_vl['ke'], 0)}% (premissa do autor) do fim de 2026, mais perpetuidade sobre 2031 a g {fmt(_vl['g'], 0)}%, menos dívida líquida de 2T26 (R$ {fmt(_vl['dl'])} mi); {fmt(_vl['nsh'], 1)} mi de ações; preço CYRE3 de 18/09/26. Sensibilidade, cenário +5% s/ LTM com permuta: Ke 13% R$ {fmt(_vl['sens']['ltm@13'], 0)}, 15% R$ {fmt(_vl['sens']['ltm@15'], 0)}, 19% R$ {fmt(_vl['sens']['ltm@19'], 0)}; g 2% R$ {fmt(_vl['sens_g']['ltm@g2'], 0)}, 6% R$ {fmt(_vl['sens_g']['ltm@g6'], 0)}. As participações em Cury, Lavvi e P&P entram pelos dividendos e pela equivalência, não a valor de mercado."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
