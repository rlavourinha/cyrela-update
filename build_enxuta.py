...
_cdb = ('<div class="viz" style="margin-top:4px">' + CRD["svg"] + '</div>' + CRD["table"] + CRD["table2"]
    + _obox(f'Só o mix contratado tira {fmt(_cd["cy_dias"] - _cd["dias31_base"], 0)} dias até 2031; a +30 pp, {fmt(_cd["cy_dias"] - _cd["dias31_p30"], 0)} dias e R$ {fmt(_cd["cx_p30_27_31"] - _cd["cx_base_27_31"], 1)} bi a mais de caixa.',
            f'Cyrela a {fmt(_cd["cy_dias"], 0)} dias, Cury a {fmt(_cd["cu_dias"], 0)}; na baixa renda, Tenda {fmt(_cd["td_dias"], 0)} e P&amp;P {fmt(_cd["pp_dias"], 0)} rodam como a Cury, Direcional {fmt(_cd["dr_dias"], 0)} e MRV {fmt(_cd["mrv_dias"], 0)} não; os puros de médio e alto padrão ficam acima (Lavvi {fmt(_cd["lv_dias"], 0)}, Trisul {fmt(_cd["tr_dias"], 0)}, Even {fmt(_cd["ev_dias"], 0)}). Com a Vivaz a {fmt(_cd["dv"], 0)} dias, o resto da Cyrela roda a {fmt(_cd["dias_map"], 0)}. A Vivaz é {fmt(_cd["mix26"], 0)}% dos lançamentos e {fmt(_cd["sh_mcmv"], 0)}% da receita: a receita segue o lançamento com três anos de atraso, então até 2028 os cenários são iguais. Geração de caixa = caixa antes do recebível no nível do LTM ({fmt(_cd["cx_ante_pct"], 0)}% da receita: release R$ {fmt(_cd["cia_ltm"] / 1000, 1)} bi + Δ recebível {fmt(_cd["dcr_ltm"] / 1000, 1)} bi) menos a variação do recebível de cada cenário; a receita cresce {fmt(_cd["g"], 0)}% ao ano.'))
_cds = sl("parte 4 · lucro, caixa e payout", f"Recebível: {fmt(_cd['cy_dias'], 0)} dias; o mix da Vivaz tira {fmt(_cd['cy_dias'] - _cd['dias31_base'], 0)} a {fmt(_cd['cy_dias'] - _cd['dias31_p30'], 0)} até 2031.", _cdb,
    nota=f"Fontes: Cyrela, planilha de DFs do RI (Economatica; contas a receber CP + LP, líquido de AVP e provisão para distrato; receita, desde 2010) e balanço CVM (PL consolidado); nota de contas a receber do ITR {_cd['nq']} (concluídas R$ {fmt(_cd['concl'], 0)} mi, em construção R$ {fmt(_cd['constr'], 0)} mi, bruto); receita por segmento (MCMV = Vivaz, 12 m). Cury: planilha Fundamentos do RI (contas a receber CP + LP, PL total, receita). Pares: ITR/DFP consolidados na CVM (contas a receber CP + LP; receita 3.01), 2021+; MRV consolida Resia e Urba, Direcional inclui Riva, Tenda, Alea. Cenários: mix de lançamentos da Vivaz de 2027 em diante (base 32% do 1S26, +10, +20 e +30 pp); receita do ano = lançamentos de t−1 a t−4 com pesos 15/35/35/15 (ciclo de 39 meses do anexo de entregues; reproduz 19% de Vivaz na receita em 2026 contra 17,7% observados); participação da Cyrela igual nos dois produtos (71-80%), então o mix a 100% vale a stake. Recebível ÷ PL da Cury alto porque o PL é pequeno, não porque o recebível é longo."   # 23/09/26: nota em 4 linhas (era 5 com os pares de baixa renda); a fórmula dos dias já está no subtítulo do painel 1
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
