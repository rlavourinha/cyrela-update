...
_cdb = ('<div class="viz" style="margin-top:4px">' + CRD["svg"] + '</div>' + CRD["table"]
    + _obox(f'No patamar da Cury, o recebível da Cyrela solta R$ {fmt(_cd["lib_a"] / 1000, 1)} bi; só com mais MCMV no mix, {fmt(_cd["lib_30"] / 1000, 1)} a {fmt(_cd["lib_40"] / 1000, 1)} bi.',
            f'A Cyrela carrega {fmt(_cd["cy_dias"], 0)} dias de receita em recebível (R$ {fmt(_cd["cy_cr"] / 1000, 1)} bi, {fmt(_cd["cy_cr_pl"], 0)}% do PL); a Cury, {fmt(_cd["cu_dias"], 0)} dias, mesmo com a venda direta; na baixa renda, Tenda {fmt(_cd["td_dias"], 0)} e P&amp;P {fmt(_cd["pp_dias"], 0)} rodam como a Cury, mas Direcional {fmt(_cd["dr_dias"], 0)} e MRV {fmt(_cd["mrv_dias"], 0)} não; os puros de médio e alto padrão ficam acima: Lavvi {fmt(_cd["lv_dias"], 0)}, Trisul {fmt(_cd["tr_dias"], 0)}, Even {fmt(_cd["ev_dias"], 0)}. O MCMV é {fmt(_cd["sh_mcmv"], 0)}% da receita da Cyrela: levar o mix a 30% ou 40% com os dias da Cury solta pouco, porque o recebível está no médio e alto padrão ({fmt(_cd["dias_map"], 0)} dias implícitos, R$ {fmt(_cd["concl"] / 1000, 1)} bi já de unidade concluída). A liberação grande exige mudar o MAP, não o mix.'))
_cds = sl("parte 4 · lucro, caixa e payout", f"Recebível: {fmt(_cd['cy_dias'], 0)} dias na Cyrela, {fmt(_cd['cu_dias'], 0)} na Cury: R$ {fmt(_cd['lib_a'] / 1000, 1)} bi presos.", _cdb,
    nota=f"Fontes: Cyrela, planilha de DFs do RI (Economatica; contas a receber CP + LP, líquido de AVP e provisão para distrato; receita trimestral, desde 2010) e balanço CVM (PL consolidado); nota de contas a receber do ITR {_cd['nq']} (concluídas R$ {fmt(_cd['concl'], 0)} mi, em construção R$ {fmt(_cd['constr'], 0)} mi, bruto); receita por segmento (MCMV = Vivaz, 12 m). Cury: planilha Fundamentos do RI (contas a receber CP + LP, PL total, receita). Pares: ITR/DFP consolidados na CVM (contas a receber CP + LP; receita 3.01), 2021+; MRV consolida Resia e Urba, Direcional inclui Riva, Tenda inclui Alea. Dias = contas a receber ÷ receita 12 m × 365. Cenários: (a) recebível = receita 12 m × dias da Cury; (b) MCMV nos dias da Cury, MAP nos dias implícitos, MCMV de {fmt(_cd['sh_mcmv'], 0)}% para 30% e 40% da receita, mesma receita. Recebível ÷ PL da Cury alto porque o PL é pequeno (distribui quase todo o lucro), não porque o recebível é longo."   # 23/09/26: nota em 4 linhas (era 5 com Lavvi/Trisul), só palavras redundantes cortadas
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
