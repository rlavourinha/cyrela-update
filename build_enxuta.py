...
# 4T25: a renúncia em ata ao direito de desistir liberou o reconhecimento de projetos represados (call 4T25; sem valor divulgado). Estimado por diferença: resíduo do 4T25 menos a média do resíduo de 3T25 e 1T26; o que sobra é a usinagem de tendência
for q in _qs37: _cq[q]["rep"] = 0.0
if "4T25" in _cq and "3T25" in _cq and "1T26" in _cq:
    _tr = (_cq["3T25"]["usin"] + _cq["1T26"]["usin"]) / 2; _cq["4T25"]["rep"] = _cq["4T25"]["usin"] - _tr; _cq["4T25"]["usin"] = _tr
...
if _cq.get("4T25", {}).get("rep"):
    _i4 = _qs37.index("4T25"); _xa = c.x(_i4); _ya, _yb = c.y(_cq["4T25"]["usin"]), c.y(_cq["4T25"]["usin"] + _cq["4T25"]["rep"])
    c.g.append(f'<line x1="{_xa:.1f}" y1="{_ya:.1f}" x2="{_xa:.1f}" y2="{_yb:.1f}" stroke="{S1}" stroke-width="1.6" stroke-dasharray="3 3"/><circle cx="{_xa:.1f}" cy="{_yb:.1f}" r="3.2" fill="none" stroke="{S1}" stroke-width="1.6"/><text x="{_xa-8:.1f}" y="{_yb-6:.1f}" class="fw-t2" fill="{S1}" text-anchor="end">represado {fmt(_cq["4T25"]["rep"], 1)}</text>')
...
def _tr37(lab, key, pct=False, cls=""):
    cells = "".join(f'<td style="text-align:right;padding:2px 3px">{("—" if key == "rep" and not _cq[q][key] else (fmt(100 * _cq[q][key] / _cq[q]["rev"], 0) + "%" if pct else fmt(_cq[q][key], 1)))}</td>' for q in _qs37)
...
body += ('<div class="viz" style="margin-top:6px"><table class="tl compact" style="width:100%;font-size:9.5px"><thead><tr><th style="text-align:left;padding:2px 6px 2px 2px">R$ bi</th>' + _th + '</tr></thead><tbody>'
         + _tr37("receita líquida (DRE)", "rev", cls="total") + _tr37("venda de lançamento (15%)", "lanc") + _tr37("venda de estoque em obra (PoC)", "obra") + _tr37("venda de estoque pronto", "pronto") + _tr37("usinagem (obra do já vendido)", "usin", cls="total") + _tr37("reconhecimento represado (4T25, est.)", "rep") + _tr37("usinagem, % da receita", "usin", pct=True)
         + '</tbody></table></div>')
...
body += _obox37(f'Usinagem: {fmt(_u37[-1], 0)}% da receita no {_qs37[-1]}, entre {fmt(min(_u37x), 0)}% e {fmt(max(_u37x), 0)}% desde {"20" + _qs37[0][2:]}; estoque em obra, {fmt(_o37[-1], 0)}%.',
              f'O pico do 4T25 não é obra: R$ {fmt(_cq["4T25"]["rep"], 1)} bi acima da tendência é reconhecimento represado, liberado pela renúncia ao direito de desistir (estimado por diferença).')
_s37 = sl(P5, "Receita por trimestre: quanto veio da venda, quanto da obra.", body, nota="Fontes: DRE (CYREMod, receita líquida trimestral); planilha do RI (vendas de lançamento, de estoque em construção e de estoque pronto, VGV 100%); ITR (nota de estoques) e releases (custo a incorrer) para o PoC do estoque em construção de cada trimestre (_usinagem.json; interpolado em 3T21 e 4T22). Premissas do slide 'De onde vem a receita': 90% das vendas no perímetro consolidado, lançamento reconhecido a 15%, pronto a 100%; usinagem = receita − parcela reconhecida na venda (piso, porque a venda de unidade lançada em trimestre anterior conta como estoque). 4T25: o CFO explicou no call que a companhia passou a renunciar em ata, a cada fechamento, ao direito de desistir dos projetos, o que liberou de uma vez o reconhecimento de lançamentos represados de trimestres anteriores, sem quantificar; aqui o represado é o resíduo do 4T25 menos a média do resíduo de 3T25 e 1T26, e antes do 4T25 a parcela de lançamento do modelo (15% na venda) está adiantada em até dois trimestres.").replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
