...
_cdb = ('<div class="viz" style="margin-top:4px">' + CRD["svg"] + '</div>' + CRD["table"]
    + _obox(f'No patamar da Cury, o recebível da Cyrela solta R$ {fmt(_cd["lib_a"] / 1000, 1)} bi; só com mais MCMV no mix, {fmt(_cd["lib_30"] / 1000, 1)} a {fmt(_cd["lib_40"] / 1000, 1)} bi.',
            f'Cyrela: {fmt(_cd["cy_dias"], 0)} dias de receita em recebível (R$ {fmt(_cd["cy_cr"] / 1000, 1)} bi, {fmt(_cd["cy_cr_pl"], 0)}% do PL); Cury, {fmt(_cd["cu_dias"], 0)}, mesmo com a venda direta; na baixa renda, Tenda {fmt(_cd["td_dias"], 0)} e P&amp;P {fmt(_cd["pp_dias"], 0)} rodam como a Cury, Direcional {fmt(_cd["dr_dias"], 0)} e MRV {fmt(_cd["mrv_dias"], 0)} não; puros de médio e alto padrão acima: Lavvi {fmt(_cd["lv_dias"], 0)}, Trisul {fmt(_cd["tr_dias"], 0)}, Even {fmt(_cd["ev_dias"], 0)}. O MCMV é {fmt(_cd["sh_mcmv"], 0)}% da receita: levar o mix a 30% ou 40% nos dias da Cury solta pouco, porque o recebível está no MAP ({fmt(_cd["dias_map"], 0)} dias implícitos, R$ {fmt(_cd["concl"] / 1000, 1)} bi já de unidade concluída). A liberação grande exige mudar o MAP, não o mix.'))   # 23/09/26: apoio em 3 linhas (era 4), só palavras redundantes cortadas
