...
_rb = ('<div class="viz" style="margin-top:6px">' + RT["svg"] + '</div>' + RT["table"] + '<div class="viz" style="margin-top:8px">' + CX["svg"] + '</div>'
    + _obox(f'Retomada em SP: de {fmt(_ra["2020"]["shr"], 1)}% para {fmt(_ra["2025"]["shr"], 1)}% das compras residenciais, e {fmt(_ra["2025"]["p350"], 0)}% delas até R$ 350 mil.',
            f'Média 12m: de {fmt(_rn["ret12_min"], 0)} retomadas por mês (mar/21) para {fmt(_rn["ret12"], 0)} (jul/26); leilões sobem menos ({fmt(_rn["arr12"], 0)}/mês), o estoque fica no banco: na Caixa, imóveis retomados de R$ {fmt(_cx["rec_min"] / 1000, 1)} bi (jun/22) a {fmt(_cx["rec_now"] / 1000, 1)} bi (jun/26), R$ {fmt(_cx["desp_2025"] / 1000, 1)} bi de despesa em 2025. Para a incorporadora, o sinal pega no pró-soluto, na venda direta e no MCMV.'))   # 21/09/26: apoio encurtado de 3 para 2 linhas
...
_sbb = ('<div class="viz" style="margin-top:6px">' + SB["svg"] + '</div>' + SB["table"]
    + _obox(f'Goodwill de 2020 é R$ {fmt(_sb_["gw_tot"] / 1000, 1)} bi do PL; a fatia na P&P: R$ {fmt(_sb_["pp_book"])} mi no book, R$ {fmt(_sb_["pp_mkt"])} mi em bolsa.',
            f'Na perda de controle a Cyrela remensurou P&P e Lavvi a valor justo (R$ {fmt(_sb_["gw_2020"])} mi de goodwill, sem amortização, só teste de valor em uso). A equivalência é a fatia do lucro contábil: {fmt(_sb_["roe_pp"], 0)}% de ROE na P&P viram {fmt(_sb_["ret_pp"], 0)}% sobre o book da Cyrela; na Lavvi, {fmt(_sb_["roe_lv"], 0)}% viram {fmt(_sb_["ret_lv"], 0)}%. Sem o goodwill o ROE da Cyrela sobe de {fmt(_sb_["roe_cy"], 1)}% para {fmt(_sb_["roe_ex_gw"], 1)}%; o risco é o inverso: impairment.'))
...
_ssb = ('<div class="viz" style="margin-top:6px">' + SS["svg"] + '</div>' + SS["table"]
    + _obox(f'Para o alvo de R$ {_ss["tp"]} o Itaú BBA precisa da VSO de estoque de {fmt(_ss["vso_est_26"], 0)}% para {fmt(_ss["vso_est_30"], 0)}% ao ano.',
            f'Lançamento cai {fmt(-_ss["lanc_var"], 0)}% e o estoque do alto padrão para em R$ 11 bi (slide 59); distratos em {fmt(_ss["cancel_26"], 0)}% das vendas brutas; dividendo zero em 2026. Na Cury, o alvo de R$ {_ss["tp_cu"]} não modela a venda direta (recebível de R$ {fmt(_ss["cu_cr_25"], 1)} para {fmt(_ss["cu_cr_30"], 1)} bi até 2030) e o consenso Bloomberg (11 casas) está no mesmo lugar: R$ 6,8 bi de receita e 1,24 bi de lucro.'))
_sss = sl("anexo · sell-side · Itaú BBA", "Sell-side: o que o Itaú BBA assume para Cyrela e Cury.", _ssb, cls="anexo",
    nota="Fontes: modelos do Itaú BBA distribuídos a clientes (fontes/sellside/CYRE_Model_1Q26_IBBA.xlsx, 25/05/26, Outperform, TP R$ 35; CURY_Model_2Q26_IBBA.xlsx, Outperform, TP R$ 44), abas Operating, IS, BS & FCF, Valuation e SOTP; valores transcritos sem ajuste; consenso Bloomberg da Cury (Standard, BRL, telas de 21/09/26, fontes/consenso_bloomberg_cury_set26.md; dividendo = DPS × 308,0 mi de ações). Coluna 'este deck' = números dos slides citados, fontes primárias. Barras claras = estimativa. Este anexo existe para confronto e pode ser retirado sem afetar o resto do deck."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
