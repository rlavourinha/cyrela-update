...
_dpb = ('<div class="viz" style="margin-top:0">' + DPP["svg"] + '</div>' + DPP["table"]
    + _obox(f'ROE de {fmt(_dp["roe_ltm"], 0)}% para {fmt(_dp["roe27"], 0)}% em 2027 e {fmt(_dp["roe31"], 0)}% em 2031: margem parada em {fmt(_dp["mg27"], 0)}%, giro caindo, ROE no Ke.',
            f'Margem constante por construção; cai o giro sobre o PL, de {fmt(_dp["gp27"], 2)}x para {fmt(_dp["gp31"], 2)}x, porque o PL cresce mais que a receita: o modelo retém R$ {fmt(_dp["ret_27_31"] / 1000, 1)} bi de lucro em recebível, obra e SPEs em 2027-31 e o PL vai de R$ {fmt(_dp["pl0"] / 1000, 1)} bi para {fmt(_dp["pl31"] / 1000, 1)} bi. Lançando o que vende, ROE de {fmt(_dp["roe31_lstar"], 0)}% em 2031; Vivaz a +30 pp, {fmt(_dp["roe31_p30"], 0)}%; sobre 2025, {fmt(_dp["roe31_2025"], 0)}%. ROE no Ke de {fmt(_dp["ke"], 0)}% é o que 1,0x book já diz. Histórico: {fmt(_dp["roe_max"], 0)}% em {_dp["roe_max_ano"]} (marcação dos IPOs), {fmt(_dp["roe_2016"], 0)}% em 2016, {fmt(_dp["roe_2019"], 0)}% em 2019.'))   # 24/09/26: apoio cortado para 3 linhas
