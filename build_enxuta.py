...
_srb = ('<div class="viz" style="margin-top:4px">' + SRM["svg"] + '</div>' + SRM["table"]
    + _obox(f'No Rio, a Cury é {fmt(_sr["c_rmrj"], 0)}% do MCMV financiado e a Vivaz {fmt(_sr["v_rmrj"], 0)}%; em São Paulo, {fmt(_sr["c_rmsp"], 0)}% e {fmt(_sr["v_rmsp"], 0)}% (12 meses).',
            f'O share estadual do slide anterior dilui as duas no interior e nos usados. Na região metropolitana, Cury e Vivaz somam {fmt(_sr["soma_rmsp_25"], 0)}% da RMSP e {fmt(_sr["soma_rmrj_25"], 0)}% da RMRJ em 2025: o Rio é o mercado onde a Cury domina ({fmt(_sr["c_rmrj_24"], 0)}% em 2024) e onde a Vivaz é a segunda praça, com {fmt(_sr["v_prj"], 0)}% dos lançamentos contra {fmt(_sr["c_prj"], 0)}% na Cury. No Brasil, {fmt(_sr["c_br"], 1)}% e {fmt(_sr["v_br"], 1)}%.'))
_srs = sl("parte 5 · atualização operacional · MCMV", "Por região: a Cury é um terço do Rio; a Vivaz, 14% de SP.", _srb,
    nota="Fontes: MCid, MCMV Financiado (FGTS/FS), unidades por município e mês de assinatura (ref. 24/07/26), somadas nos 39 municípios da RMSP, nos 22 da RMRJ e no país; inclui todas as faixas e imóveis usados. Vivaz: anexo 'Lançamentos' dos releases da Cyrela (4T17-4T25 e 2T26: empreendimento, região, unidades 100%, produto MCMV/CVA Faixas 2 e 3), ex-Plano & Plano; 2026 estimado pelo ticket do RI; bate com a planilha do RI ano a ano. Cury: unidades lançadas da planilha do RI (100%) rateadas pela contagem de empreendimentos por praça dos releases (4T22+); a Cury não abre unidades por cidade, então SP e RJ são estimativas. 'Peso do Rio' = unidades no RJ ÷ (SP + RJ). LTM = 3T25-2T26."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
