...
# --- projeção de caixa 2027-31 com as linhas da reconciliação, três cenários de lançamentos (24/09/26); dados de grafico_caixa_proj.py
CXP = J("_caixa_proj_frag.json"); _cp = CXP["num"]
_cpb = ('<div class="viz" style="margin-top:0">' + CXP["svg"] + '</div>' + CXP["table"]
    + _obox(f'Permuta: R$ {fmt(_cp["cx_ltm_29"] / 1000, 1)} bi em 2029 lançando +5%; terrenista exigindo caixa: {fmt(_cp["cx_ltm_tc_29"] / 1000, 1)} bi; corte: {fmt(_cp["cx_corte_29"] / 1000, 1)} bi.',
            f'Terreno em permuta (o saldo a pagar de R$ 3,2 bi rola com compras a prazo) ou em caixa: parcelas da nota sem reposição e terreno novo metade à vista (R$ {fmt(_cp["cx_ltm_29"] / 1000 - _cp["cx_ltm_tc_29"] / 1000, 1)} bi a menos em 2029). Em 2027 o recebível cresce R$ {fmt(-_cp["cr_ltm_27"] / 1000, 1)} bi (receita contratada +{fmt(100 * (_cp["rec_27"] / _cp["rec_ltm_27_base"] - 1), 0)}%); o corte de 30% libera obra e recebível e custa receita de 2029 em diante ({fmt(_cp["rec_corte_29"] / 1000, 1)} bi vs {fmt(_cp["rec_ltm_29"] / 1000, 1)}). 2023-25 gerou {fmt(_cp["cia_23_25"][0], 0)}, {fmt(_cp["cia_23_25"][1], 0)} e {fmt(_cp["cia_23_25"][2], 0)} mi; a projeção fica acima porque o lançamento para de crescer 40% ao ano e a obra, de consumir caixa.'))   # 25/09/26: apoio em 3 linhas (receita {frec}% e obra {beta}% já estão na coluna 'premissa' da tabela)
_cps = sl("parte 4 · lucro, caixa e payout", f"Caixa 2029: R$ {fmt(_cp['cx_ltm_29'] / 1000, 1)} bi com permuta, {fmt(_cp['cx_ltm_tc_29'] / 1000, 1)} bi pagando terreno.", _cpb,
    nota=f"Fontes: reconciliação do slide anterior (planilha de DFs do RI, notas dos ITR, balanço CVM, releases; R$ mi); lançamentos do RI (VGV 100%). Cenários: (1) +{fmt(_cp['g'], 0)}% ao ano sobre o LTM 2T26; (2) +5% sobre 2025; (3) −30% em 2027 (entre o −15% do Itaú BBA para 2026 e o −50% de 2015-16), 2028 estável, depois +5%; (1) com a Vivaz a +30 pp. Receita calibrada no LTM (2020-25: ~100%); % da receita = média 2023-LTM. Terrenos: permuta = 0 (2019-22 a linha foi −1% a −2% dos lançamentos; 2023-LTM, positiva pelo terrenista) ou em caixa = terreno no CPV (18% da receita MAP) − parcelas da nota do ITR (R$ 3,2 bi em 2T26) − terreno dos lançamentos de t+1, ½ à vista, ½ em dois anos (2013, compra à vista: −12% dos lançamentos). Sem dividendos, recompra, venda de participações e juros além do financeiro da DRE."   # 25/09/26: nota em 4 linhas (a cláusula 'caixa pago' repetia a do terreno de t+1)
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
...
# --- valuation: lucro projetado, fluxo ao acionista e VPL por cenário (24/09/26); dados de grafico_valuation.py
VLU = J("_valuation_frag.json"); _vl = VLU["num"]
_vlb = ('<div class="viz" style="margin-top:0">' + VLU["svg"] + '</div>' + VLU["table"] + VLU["table2"]
    + _obox(f'A Ke de {fmt(_vl["ke"], 0)}%, o fluxo vale R$ {fmt(_vl["ps_ltm"], 0)} por ação lançando +5% e {fmt(_vl["ps_corte"], 0)} no corte; a tela paga {fmt(_vl["px"], 2)}.',
            f'Lucro líquido de R$ {fmt(_vl["ll27"] / 1000, 1)} bi em 2027 e {fmt(_vl["ll29"] / 1000, 1)} bi em 2029 (P/L {fmt(_vl["pe27"], 1)}x e {fmt(_vl["pe29"], 1)}x), mas só {fmt(_vl["conv_ll_27_31"], 0)}% vira caixa em 2027-31. O VPL só encosta no preço se a Cyrela parar de crescer (corte: R$ {fmt(_vl["ps_corte"], 0)}) ou se o Ke cair a {fmt(13, 0)}% (R$ {fmt(_vl["sens"]["ltm@13"], 0)}); com o terrenista exigindo caixa, R$ {fmt(_vl["ps_ltm_tc"], 0)}. O mercado paga {fmt(_vl["pb"], 1)}x o book e {fmt(_vl["pe_ltm"], 1)}x o lucro do LTM: está precificando a conversão de 2013-19, não a de 2020-25.').replace('<div class="sl-output" style="row-gap:3px;margin-top:6px">', '<div class="sl-output" style="row-gap:3px;margin-top:6px;padding:7px 16px">', 1))   # 25/09/26: caixa com padding 7px (slide a 751px; alvo 720)
_vls = sl("parte 4 · lucro, caixa e payout", f"Valuation: R$ {fmt(_vl['ps_ltm'], 0)} a {fmt(_vl['ps_corte'], 0)} por ação a Ke de {fmt(_vl['ke'], 0)}%, contra {fmt(_vl['px'], 0)} na tela.", _vlb,
    nota=f"Fontes: modelo de caixa do slide anterior (receita e lucro bruto por cenário); linhas abaixo do lucro bruto em % da receita na média de 2023-LTM (coluna premissa), conferidas no LTM (lucro líquido R$ {fmt(_vl['ll_ltm'])} mi). Fluxo ao acionista = geração de caixa operacional projetada (já com financeiro, dividendos das JVs, investimentos e minoritários; sem dividendos pagos e recompra). VPL = fluxos de 2027-31 descontados a Ke {fmt(_vl['ke'], 0)}% (premissa do autor) do fim de 2026, mais perpetuidade sobre 2031 a g {fmt(_vl['g'], 0)}%, menos dívida líquida de 2T26 (R$ {fmt(_vl['dl'])} mi); {fmt(_vl['nsh'], 1)} mi de ações; preço CYRE3 de 18/09/26. Sensibilidade, +5% s/ LTM com permuta: Ke 13% R$ {fmt(_vl['sens']['ltm@13'], 0)}, 15% R$ {fmt(_vl['sens']['ltm@15'], 0)}, 19% R$ {fmt(_vl['sens']['ltm@19'], 0)}; g 2% R$ {fmt(_vl['sens_g']['ltm@g2'], 0)}, 6% R$ {fmt(_vl['sens_g']['ltm@g6'], 0)}. Cury, Lavvi e P&P entram pelos dividendos e pela equivalência, não a valor de mercado."   # 25/09/26: nota em 4 linhas (os % das premissas já estão na coluna da tabela)
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
