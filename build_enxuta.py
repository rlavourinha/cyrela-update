...
def _mslide(seg, title, card_n, card_txt, out_msg, out_sub, h2px):
    body = ('<div class="viz" style="margin-top:0">' + MF[seg]["svg"] + '</div>'
            '<div class="fwgrid" style="grid-template-columns:1.1fr 1fr;gap:14px;margin-top:4px;align-items:center"><div class="viz" style="margin-top:0">' + MF[seg]["secovi"] + '</div>'
            ...
            .replace('<span class="out-msg">', '<span class="out-msg" style="flex:1 1 600px;line-height:1.2">', 1).replace('<span class="out-sub">', '<span class="out-sub" style="flex:1 1 100%;line-height:1.35">', 1))
    return sl("parte 5 · atualização operacional · mercado", title, body, nota="Fontes: ABRAINC-FIPE, Indicadores do Mercado Imobiliário (lançamentos e vendas líquidas de distratos por segmento, 12 meses, abr/26); Secovi-SP, PMI jun/26 (cidade de São Paulo, empresas associadas; MCMV = Faixas 1-3 pelos limites de abr/26; VGV a INCC de jun/26); Cyrela, planilha de dados operacionais do RI (VGV lançado 100%, 12 meses; lista de empreendimentos, praça São Paulo = capital e região metropolitana, unidades MCMV 100%). Rio de Janeiro: a Ademi-RJ/Brain publica só o agregado (2025: R$ 17,6 bi lançados, +37%; 1S26: 13,3 mil unidades lançadas, +2%), sem abertura por padrão."
...
_sc = _mslide("mcmv", "MCMV: o mercado segue no recorde, com funding próprio.",
    ...
    f'<b>Brasil</b>: lançadas e vendidas em 12 meses, recorde da série. <b>São Paulo</b>: 81% do lançado e 75% do vendido (jun/26); VSO 11,6% contra 5,7% do resto. <b>Cyrela (Vivaz)</b>: R$ {fmt(_n["cy_mcmv_ult"][1], 1)} bi lançados em 12 meses, recorde; na praça São Paulo, 15,5 mil unidades em 12 meses, ~16% das 96,5 mil do MCMV lançadas na capital (Secovi).',
    "O MCMV não desacelera: FGTS e orçamento do programa, não a Selic, decidem o volume.", "É o segmento que segura o lançado da Cyrela em 2026, com a margem exposta ao INCC e à Caixa.", 37)   # 19/09/26: título em uma linha a 37px (1005px; a 38px dava 1031 > 1020)
