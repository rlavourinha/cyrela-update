...
_s48 = sl("parte 4 · onde estamos no ciclo", f"Cury: {fmt(_n2['pb_cury'], 1)}x book com ROE de {fmt(_n2['roe_cury'], 0)}%; a Vivaz valeria {fmt(_n2['pb_viv'], 1)}x a {fmt(_n2['pb_viv_top'], 1)}x.",
    ...
    + _obox(f'Alavancagem operacional: caixa líquido, terreno a prazo = {fmt(_n2["cred_pl"], 0)}% do PL; ROE {fmt(_n2["roe_cury"], 0)}% vale {fmt(_n2["pb_cury"], 1)}x book.',
            f'Vivaz: piso {fmt(_n2["pb_viv"], 1)}x (Gordon), teto {fmt(_n2["pb_viv_top"], 1)}x com o já vendido ({fmt(_n2["vr_viv"], 1)}x a receita; Cury {fmt(_n2["vr_cury"], 1)}x): R$ {fmt(_n2["val_viv"], 1)} a {fmt(_n2["val_viv_top"], 1)} bi, {fmt(100 * _n2["val_viv"] / _n2["mc_cyre"], 0)} a {fmt(100 * _n2["val_viv_top"] / _n2["mc_cyre"], 0)}% da Cyrela com 8% do PL.'),
    nota=f"Fontes: Cury, planilha Fundamentos do RI (DRE e balanço trimestrais, consolidado): retorno operacional = lucro antes do resultado financeiro LTM ÷ PL total médio de 5 pontas; ROE = lucro atribuído aos controladores LTM ÷ PL da controladora médio; dívida líquida = empréstimos e financiamentos − caixa − títulos; terreno a prazo = credores por imóveis compromissados. B3, preços e ações em {_n2['mkt']['data']}. Vivaz: nota de segmentos do ITR (2T26), sem juros, IR e minoritários alocados; ROE estimado = retorno operacional × {fmt(_n2['fator_ll'], 2)} (razão ROE ÷ retorno operacional da Cury). P/B implícito = (ROE − g) ÷ (Ke − g), Ke 17% e g 4%, premissas do slide do P/B; a Cury a mercado ({fmt(_n2['pb_cury'], 1)}x) confirma a régua. Teto = dois estágios: PL cresce {fmt(100 * _n2['g1'])}% a.a. por {_n2['n1']} anos reinvestindo (payout = 1 − g ÷ ROE), depois perpetuidade a g 4% com o mesmo ROE; 25% a.a. é o que a venda já contratada sustenta. Vendas ÷ receita LTM: Cury, vendas líquidas parte Cury ÷ receita (RI); Vivaz, vendas 100% do segmento ÷ receita do segmento (ITR). Não é preço-alvo.").replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
