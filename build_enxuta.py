...
_s8_add = ('<div class="fwgrid" style="grid-template-columns:1.15fr 1fr;margin-top:6px;gap:10px;align-items:start">' + re.sub(r' style="[^"]*"', ' style="margin-top:0"', _viz41, count=1)
    ...
    + '<div class="viz" style="margin-top:6px"><table class="tl" style="width:100%;font-size:10px;line-height:1.1;border:2px solid var(--s2);border-collapse:collapse;text-align:center"><thead>'
       '<tr style="background:var(--s2);color:#fff"><th rowspan="2" style="text-align:left;padding:2px 6px;vertical-align:middle;width:27%">retorno consolidado = w × Vivaz + (1 − w) × médio e alto padrão</th><th colspan="4" style="padding:2px 6px;border-left:1px solid rgba(255,255,255,.5)">Vivaz a 40% (hoje)</th><th colspan="4" style="padding:2px 6px;border-left:1px solid rgba(255,255,255,.5)">Vivaz a 30% (margem cai a 30%)</th></tr>'
       ...
       + ''.join('<tr' + (' style="background:rgba(70,110,170,.12);font-weight:600"' if w == 0.2 else '') + f'><td style="text-align:left;padding:2px 6px">Vivaz com {fmt(100 * w, 0)}% do capital{" (cenário; hoje 11%)" if w == 0.2 else ""}</td>' + ''.join(f'<td style="padding:2px 5px{";border-left:1px solid var(--s2)" if k == 0 else ""}">{fmt(w * rv + (1 - w) * rm, 1)}%</td>' for rv in (40, 30) for k, rm in enumerate((23, 15, 8, 3.6))) + '</tr>' for w in (0.2, 0.33, 0.5))
