...
_cpb = ('<div class="viz" style="margin-top:0">' + CXP["svg"] + '</div>' + CXP["table"]
    + _obox(f'2027 perto de zero em todos os cenários; 2029: R$ {fmt(_cp["cx_ltm_29"] / 1000, 1)} bi lançando +5% e {fmt(_cp["cx_corte_29"] / 1000, 1)} bi com o corte.',
            f'Receita = {fmt(_cp["frec"], 0)}% dos lançamentos de t−1 a t−4 (pesos 15/35/35/15; o LTM reconhece menos porque 2025-26 vendeu mais devagar); estoque de obra a custo = {fmt(_cp["beta"], 0)}% da média de três anos de lançamentos; demais linhas em % da receita no nível de 2023-LTM; terreno neutro. Em 2027 o recebível cresce R$ {fmt(-_cp["cr_ltm_27"] / 1000, 1)} bi porque a receita já contratada sobe {fmt(100 * (_cp["rec_27"] / _cp["rec_ltm_27_base"] - 1), 0)}%; o corte de 30% ({fmt(_cp["lanc_corte_27"] / 1000, 1)} bi em 2027) libera obra e recebível e custa receita a partir de 2029 ({fmt(_cp["rec_corte_29"] / 1000, 1)} bi contra {fmt(_cp["rec_ltm_29"] / 1000, 1)}). Em 2023-25 a companhia gerou {fmt(_cp["cia_23_25"][0], 0)}, {fmt(_cp["cia_23_25"][1], 0)} e {fmt(_cp["cia_23_25"][2], 0)} mi: a projeção fica acima porque o lançamento para de crescer 40% ao ano, e a obra, de consumir caixa.'))
_cps = sl("parte 4 · lucro, caixa e payout", f"Caixa: zero em 2027 e R$ {fmt(_cp['cx_ltm_29'] / 1000, 1)} bi em 2029 lançando +5%.", _cpb,
    nota=f"Fontes: reconciliação do slide anterior (planilha de DFs do RI, notas dos ITR, balanço CVM, releases; R$ mi); lançamentos do RI (VGV 100%). Cenários: (1) lançamentos +{fmt(_cp['g'], 0)}% ao ano sobre o LTM 2T26 (R$ {fmt(_cp['lanc_ltm'] / 1000, 1)} bi); (2) +5% ao ano sobre 2025 (R$ {fmt(_cp['lanc_25'] / 1000, 1)} bi); (3) −30% em 2027 (entre o −15% do Itaú BBA para 2026 e o −50% de 2015-16), 2028 estável, depois +5%; (1) também com a Vivaz a +30 pp nos lançamentos. Receita = {fmt(_cp['frec'], 0)}% × Σ lançamentos t−1..t−4 (15/35/35/15), calibrado no LTM (2020-25 a razão foi ~100%); Δ estoque de obra = −Δ({fmt(_cp['beta'], 0)}% × média de 3 anos de lançamentos); Δ contas a receber pelos dias (Vivaz 160, resto {fmt(_cp['dias_map'], 0)}, mix 32% ou +30 pp); lucro bruto {fmt(_cp['mg'], 1)}%, SG&A {fmt(_cp['sga'], 1)}%, impostos, financeiro, outras, dividendos de JVs, investimentos e minoritários em % da receita (2023-LTM); terrenos neutros (2023-LTM +10,7% da receita; 2019-22 −2,1%). Sem dividendos, recompra, venda de participações e juros além do financeiro da DRE."
    ).replace('<h2 class="head-xl">', '<h2 class="head-xl" style="font-size:38px;margin-bottom:4px">', 1)
