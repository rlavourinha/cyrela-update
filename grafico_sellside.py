...
rows = [("Ke / g", f"{fmt(VAL['cy']['ke'], 1)}% / {fmt(VAL['cy']['g'], 1)}%", "17% / 4%", f"{fmt(VAL['cu']['ke'], 1)}% / {fmt(VAL['cu']['g'], 1)}%", "17% / 4%"),
        ...
        ("VSO de lançamento / de estoque", f"{fmt(CY['vso_lanc'][2], 0)}% / {fmt(CY['vso_est'][2], 0)}% → {fmt(CY['vso_est'][-1], 0)}% ao ano em 2030", "estoque: 11%/tri = 38%/ano (slide 59)", f"{fmt(CU['vso_lanc'][3], 0)}% / n.d.", "45% reportada; 25-36% sem venda direta"),
        ("distratos, % das vendas brutas", f"{fmt(CY['cancel'][2], 0)}%", "provisão = 7% do recebível (slide 61)", f"{fmt(CU['cancel'][3], 0)}%", "n.d."),
        ...
        ("lucro 2026E / 2027E", f"R$ {fmt(CY['ll'][2], 2)} / {fmt(CY['ll'][3], 2)} bi (ROE {fmt(CY['roe'][2], 1)}% / {fmt(CY['roe'][3], 1)}%)", "LTM 2,04 bi; ROE 19,5% (ajustado 16,7%)", f"R$ {fmt(CU['ll'][3], 2)} / {fmt(CU['ll'][4], 2)} bi (ROE {fmt(CU['roe'][3], 0)}%)", "LTM ROE 66%"),
        ("dividendo 2026E / 2027E", f"R$ {fmt(CY['div'][2], 2)} / {fmt(CY['div'][3], 2)} bi", "2025 pagou 1,39 bi (slide 52)", f"R$ {fmt(CU['div'][3], 2)} / {fmt(CU['div'][4], 2)} bi", "—"),
        ...
        ("Cyrela ex-sócias e ex-Vivaz", f"ROE {fmt(VAL['cy']['stand_roe'], 1)}%, negocia a {fmt(VAL['cy']['stand_ptbv'], 2)}x TBV, justo {fmt(VAL['cy']['stand_fair'], 2)}x", "ROE 16%, 0,85x (slide 50)", "—", "—"),
...
table = ('<table class="tl compact" style="width:100%;margin-top:0"><thead><tr><th style="text-align:left">premissa</th><th style="text-align:left">Itaú BBA · Cyrela (1Q26)</th><th style="text-align:left;color:var(--ink-2)">este deck</th><th style="text-align:left">Itaú BBA · Cury (2Q26)</th><th style="text-align:left;color:var(--ink-2)">este deck</th></tr></thead><tbody>'
         + "".join(f'<tr><td>{a}</td><td>{b}</td><td style="color:var(--ink-2)">{c}</td><td>{d}</td><td style="color:var(--ink-2)">{e}</td></tr>' for a, b, c, d, e in rows) + "</tbody></table>")
