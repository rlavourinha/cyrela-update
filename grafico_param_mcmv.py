...
def dec(d):   # "2004-12-14" → 2004.95
    y, m = int(d[:4]), int(d[5:7]) if len(d) >= 7 else 6
    return y + (m - 0.5) / 12
def pre(key):
    out = []
...
TETO = pre("teto_imovel_rmsp") + [(2009.3, 130), (2011.1, 170), (2012.8, 190), (2016.2, 225), (2017.0, 240), (2021.6, 264), (2025.8, 275)]
TETO_F3 = [(2023.3, 350), (2026.1, 400)]; TETO_F4 = [(2025.1, 500), (2026.1, 600)]; TETO_BANDA = [(2017.0, 300), (2020.5, 300)]
JMIN = pre("juros_min") + [(2009.3, 6.0), (2016.2, 7.0)]
JMAX = pre("juros_max") + [(2011.0, 8.16), (2021.7, 7.66), (2022.8, 8.16)]
...
R_F2 = [(2009.2, 2790), (2011.5, 3100), (2016.1, 3600), (2017.1, 4000), (2023.5, 4400), (2024.6, 4700), (2026.3, 5000)]
...
R_F4 = [(2025.4, 12000), (2026.3, 13000)]; R_DESC = pre("renda_limite_desconto")
...
def chart(H, y0, yb, vmax, vmin, grid, gfmt, title, sub, series, notes, leaders, foot=None):
    ...
    for gv in grid: g.append(f'<line x1="70" y1="{xy(A0, gv)[1]:.1f}" x2="830" y2="{xy(A0, gv)[1]:.1f}" stroke="var(--grid)" opacity=".5"/><text x="64" y="{xy(A0, gv)[1]+4:.1f}" class="axq" text-anchor="end" opacity=".8">{gfmt(gv)}</text>')
    ...
    g.append(f'<line x1="70" y1="{y0}" x2="830" y2="{y0}" stroke="var(--baseline)"/>')
    # marco do MCMV
    ...
    for a, v, txt, col, dy, anc in notes:
        x, y = xy(a, v); g.append(f'<text x="{x:.1f}" y="{y+dy:.1f}" class="fw-t2" fill="{col}" text-anchor="{anc}">{txt}</text>')
    for v, txt, col, ty in leaders:
        x, y = xy(A1, v); g.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="4" fill="{col}" stroke="var(--page)" stroke-width="2"/><line x1="{x:.1f}" y1="{y:.1f}" x2="763" y2="{ty:.1f}" stroke="{col}" stroke-width="1" opacity=".55"/><text x="826" y="{ty+4:.1f}" class="fw-t2" fill="{col}" text-anchor="end">{txt}</text>')
    if foot: g.append(f'<text x="450" y="{H-8}" class="fw-s2" text-anchor="middle">{foot}</text>')
...
svg_teto = chart(280, 250, 40, 640, 0, (150, 300, 450, 600), lambda v: fmt(v), f"O teto do imóvel — {2026 - int(A0)} anos de degraus", "R$ mil · RMSP, topo da tabela · CCFGTS",
    ...
    [(2018.7, 300, "banda estendida 2017–20 · renda 9 mil · proto-F4 (Res. 836/17)", I2, -10, "middle")],
...
svg_juro = chart(240, 210, 40, 10.8, 3.2, (4, 6, 8, 10), lambda v: f"{v}%", "O juro do programa — quase imóvel", "% a.a. + TR · topo da faixa · não-cotista · Sudeste",
    ...
    [(2015.0, 9.0, "8,16% desde 2011 — só piscou no corte de 0,5 p.p. de 2021–22", I2, 0, "middle")],
...
svg_renda = chart(259, 218, 40, 14000, 0, (4000, 8000, 12000), lambda v: f"{v // 1000} mil", "Até onde vai cada faixa — o limite de renda", "R$ mil/mês nominais · fim de cada faixa · antes de 2009: limite do FGTS (Res. CCFGTS)",
    [(R_F1, MU, 2.4, ' stroke-dasharray="3 4"', False), (R_F2, S2, 2.4, ' stroke-dasharray="6 4"', False), (R_F3, S3, 2.4, "", False), (R_F4, S1, 2.4, "", False), (R_DESC, I2, 1.8, ' stroke-dasharray="2 3"', False)],
    [(2017.1, 9000, "2017: F3 vai a 9 mil — o proto-F4", S3, -24, "middle"), (2020.7, 7000, "2020: o CVA corta a F3 para 7 mil", S3, 44, "middle")],
...
svg_desc = chart(259, 218, 40, 70000, 0, (20000, 40000, 60000), lambda v: f"{v // 1000} mil", "O desconto do FGTS — quanto o programa paga da entrada", "R$ mil · subsídio máximo por faixa nas capitais SP/RJ/DF · marcos principais",
    [(DESC_F2, S2, 2.4, ' stroke-dasharray="6 4"', False), (DESC, S2, 2.4, "", False), (DESC_N, S3, 2.4, ' stroke-dasharray="2 3"', False)],
    [(2016.1, 45000, "2016: nasce a F1,5 — subsídio dobra para quem ganha até 2.350", S2, -16, "end"), (2011.0, 23000, "2009-15: F1 e F2 com o mesmo desconto (23 → 25 mil)", I2, 24, "start")],
    [(65000, "Norte · 65 mil (nov/25)", S3, 218 - (218 - 40) * 65000 / 70000 + 20), (55000, "F1 e F2 · 55 mil", S2, 218 - (218 - 40) * 55000 / 70000 + 16)],
