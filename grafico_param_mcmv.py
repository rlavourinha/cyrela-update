...
_rd = pre("renda_limite_desconto"); R_DESC = []   # limite de renda do desconto do FGTS: só antes do MCMV (depois, o desconto segue as faixas); polyline em degraus terminando em abr/09
for _i, (_a, _v) in enumerate(_rd):
    if _i: R_DESC.append((_a, _rd[_i - 1][1]))
    R_DESC.append((_a, _v))
if R_DESC: R_DESC.append((2009.25, R_DESC[-1][1]))
...
svg_renda = chart(259, 218, 40, 14000, 0, (4000, 8000, 12000), lambda v: f"{v // 1000} mil", "Até onde vai cada faixa — o limite de renda", "R$/mês nominais · fim de cada faixa · pré-2009: FGTS",
    ...
    [(2019.5, 9000, "2017: F3 vai a 9 mil (proto-F4); 2020: o CVA corta a 7 mil", S3, -24, "end"), (2003.5, 4500, "FGTS: 1.560 → 4.900; pontilhado = renda com desconto", S3, -14, "middle")],
