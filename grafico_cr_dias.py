...
lines = [("Cyrela · contas a receber, R$ mi", [CY[q]["cr"] for q in cols], 0, ""), ("Cyrela · receita 12 m, R$ mi", [CY[q]["rec12"] for q in cols], 0, ""), ("Cyrela · dias", [CY[q]["dias"] for q in cols], 0, ""), ("Cyrela · dias, trimestre × 4", [D4_CY[q] for q in cols], 0, ""), ("Cyrela · recebível ÷ PL", [CY[q]["cr_pl"] for q in cols], 0, "%"),
         ("Cury · contas a receber, R$ mi", [CU.get(q, {}).get("cr") for q in cols], 0, ""), ("Cury · receita 12 m, R$ mi", [CU.get(q, {}).get("rec12") for q in cols], 0, ""), ("Cury · dias", [CU.get(q, {}).get("dias") for q in cols], 0, ""), ("Cury · dias, trimestre × 4", [D4_CU.get(q) for q in cols], 0, ""), ("Cury · recebível ÷ PL", [CU.get(q, {}).get("cr_pl") for q in cols], 0, "%"),
