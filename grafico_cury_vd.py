...
for q in qs:
    ...
    # 2ª curva (21/09/26): a venda direta sai do numerador mas NÃO volta ao estoque; a VSO ajustada mede o que o banco financia sobre o estoque reportado + lançamentos
    if q0 in EC: VSOX2[q] = 100 * VL[q] * (1 - D[q]) / (EC[q0] + LC[q])
...
panel(760, 955, "VSO do trimestre, %", "Cury reportada e ex-venda direta (est.); Vivaz", 60, (0, 20, 40, 60), lambda t: f"{t}%",
      [([VSO[q] for q in qs], S3, 2.6, "", "Cury", lambda v: fmt(v) + "%"), ([VSOX2.get(q) for q in qs], S3, 2.2, "2 3", "ex-direta", lambda v: fmt(v) + "%"), ([VSOX.get(q) for q in qs], S3, 2.2, "5 3", "ex-direta, +carteira", lambda v: fmt(v) + "%"), ([VZ[q] for q in qs], S1, 2.4, "", "Vivaz", lambda v: fmt(v) + "%")], qs)
...
table = ('<table class="tl compact" style="width:100%;margin-top:0"><thead><tr><th style="text-align:left">R$ bi</th>' + "".join(f'<th style="text-align:right">{q}</th>' for q in cols) + '</tr></thead><tbody>'
         ...
         + row("VSO reportada (líquida)", lambda q: VSO[q], pct=True, d=0, cls="total") + row("VSO ex-venda direta, sobre o estoque reportado (est.)", lambda q: VSOX2.get(q), pct=True, d=0, cls="total") + row("VSO ex-venda direta, carteira devolvida ao estoque (est.)", lambda q: VSOX.get(q), pct=True, d=0) + row("VSO Vivaz, mesma base", lambda q: VZ[q], pct=True, d=0) + "</tbody></table>")
u = qs[-1]; m4 = lambda d: sum(d[q] for q in qs[-4:]) / 4
num = {"u": u, "vd": VD[u], "vd0": VD[xs[0]], "ps": PS[u], "tot": TOT[u], "d": 100 * D[u], "d0": 100 * D[qs[0]], "vso": VSO[u], "vsox": VSOX[u], "vsox2": VSOX2[u], "vz": VZ[u], "vso_m4": m4(VSO), "vsox_m4": m4(VSOX), "vsox2_m4": m4(VSOX2), "vz_m4": m4(VZ), "pago": PAGO, "nq": NQ, "x0": xs[0]}
