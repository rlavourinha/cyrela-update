...
table = ('<table class="tl compact" style="width:100%;margin-top:0;font-size:9.5px"><thead><tr><th style="text-align:left">R$ mi</th>' + "".join(f'<th style="text-align:right">{a}</th>' for a in anos) + '</tr></thead><tbody>'
         + "".join(f'<tr class="{"total" if k in ("lucro_bruto", "caixa_rec", "cia_oper") else ""}"><td>{l}</td>' + "".join(f'<td style="text-align:right">{fmt(PER[a][k])}</td>' for a in anos) + "</tr>" for k, l in LAB) + "</tbody></table>")
...
json.dump({"svg": svg, "table": table, "num": num}, io.open(os.path.join(here, "_caixa_reconc_frag.json"), "w", encoding="utf-8"), ensure_ascii=False)
