...
# 25/09/26: cabeçalho em duas linhas fixas (<br> depois do período, nowrap), rótulo de linha numa linha, 9px e sem padding vertical: o slide estava em 808px com os rótulos das fases em 2-3 linhas
HDR2 = {"2013-16 · colheita do legado": "2013-16<br>colheita", "2017-19 · reconstrução": "2017-19<br>reconstrução", "2020-22 · IPOs e retomada": "2020-22<br>IPOs/retomada", "2023-25 · expansão": "2023-25<br>expansão", "2013-25 · total": "2013-25<br>total", U: U.replace(" ", "<br>"),
        "2027-31 · +5%, permuta": "2027-31<br>+5% permuta", "2027-31 · +5%, terreno em caixa": "2027-31<br>+5% terr. caixa", "2027-31 · corte 30%, permuta": "2027-31<br>corte permuta"}
PAD2 = "padding:0 5px;font-size:9px;line-height:1.15;white-space:nowrap"
table2 = ('<table class="tl compact" style="width:100%;margin-top:0"><thead><tr><th style="text-align:left;' + PAD2 + '">R$ mi, somas por fase</th>' + "".join(f'<th style="text-align:right;{PAD2}{";border-left:1px solid var(--grid)" if lab.startswith("2027") and i == len(AGG) else ""}">{HDR2.get(lab, lab)}</th>' for i, (lab, _) in enumerate(COLS)) + '</tr></thead><tbody>'
...
for i, (lab, d) in enumerate(items2):
    ...
    for j, part in enumerate(short): g2.append(f'<text x="{x + gw2/2:.1f}" y="{Y1b + 12 + 10*j}" text-anchor="middle" class="axq" opacity=".8" style="font-size:9px">{part}</text>')
g2.append(f'<line x1="{X0b}" y1="{y2(0):.1f}" x2="{X1b}" y2="{y2(0):.1f}" stroke="var(--baseline)"/>')
svg2 = '<svg viewBox="0 0 1060 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;display:block">' + "".join(g2) + "</svg>"
