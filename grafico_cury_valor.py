...
rows2 = [("preço × ações", f"{fmt(MKT['CURY3'], 2)} × {fmt(MKT['acoes_cury'] / 1e6)} mi", "—"), ("valor de mercado", f"R$ {fmt(mc_cury, 1)} bi", f"R$ {fmt(val_viv, 1)} bi (implícito; cenário {fmt(val_viv_top, 1)} bi)"), ("PL (controladora)", f"R$ {fmt(pl_ctrl, 2)} bi", f"R$ {fmt(V['pl'], 2)} bi (segmento)"),
         ("ROE", f"{fmt(cu['roe'])}%", f"~{fmt(V['roe_est'])}% (op. {fmt(V['ret_op'])}% × {fmt(fator_ll, 2)})"), ("vendas ÷ receita, LTM", f"{fmt(RC[QC[-1]], 1)}x (em regime)", f"{fmt(RV[sq[-1]], 1)}x (vendido a reconhecer)"), ("P/B de mercado", f"{fmt(pb_cury, 1)}x", "—"),
         (f"por analogia: Gordon, Ke {fmt(MKT['ke'])}% e g {fmt(MKT['g'])}%", f"{fmt(pb_cury_g, 1)}x", f"{fmt(pb_viv, 1)}x · R$ {fmt(val_viv, 1)} bi"), ("cenário: se a Vivaz chegasse ao lançamento da Cury", "—", f"{fmt(pb_viv_top, 1)}x · R$ {fmt(val_viv_top, 1)} bi"),   # 22/09/26: "teto" vira cenário rotulado (decisão editorial); os dois estágios (+20% a.a. por 5 anos, depois 4%) ficam na nota do slide
         ("Cyrela: valor de mercado", f"R$ {fmt(mc_cyre, 1)} bi", f"{fmt(100 * val_viv / mc_cyre)}% do valor (cenário {fmt(100 * val_viv_top / mc_cyre)}%); 11% do PL"), ("15,08% da Cury na mão da Cyrela", f"R$ {fmt(stake, 2)} bi", "—")]
